...
## Hermes first-minute handoff — `76b2f7d` (2026-07-15)

The committed fairness correction is healthy under board variance. Its eight-seed desktop/mobile matrix passes, the previously failing `bloodroot-moon` sequence completes, and an independent eight-seed audit matrix also completed all 16 journeys. Across that independent set, Round 1 took 3-4 swaps, Round 2 took 4-7, and Round 3 took 2-6; the tightest successful Round 2/3 cases retained one move. The stale-locator retry is also correctly bounded to no-move-spent rerenders. Do not reopen the seed matrix, tile art, cascade timing, mobile hierarchy, ceremonies, or deployment surfaces.

One materially distinct real-player strategy weakness remains. An audit strategy that still follows bouquet targets and Cursed Thorn adjacency, but does not award itself the test bot's four-match or total-cell optimization bonuses, loses the `vesper-thorn` Round 3 on both desktop and exact `390×844`: Bloodroot `13/14`, Sol Rot `13/13`, Bouquet `26/27`, Moves `0`. This player is following the stated objective and is short by one Bloodroot, not playing randomly. Make the minimum Round 3 forgiveness correction so ordinary target-following play does not require hidden move-value optimization. Prefer a deficit-aware hint or one additional reachable Bloodroot opportunity; change the seven-move budget only if the board-level correction cannot keep the result robust. Do not add UI, rounds, systems, boosters, controls, objectives, or progression.

Acceptance checks:

- Add a secondary deterministic goal-following strategy check that scores target matches and Round 2 thorn adjacency but does not score four-match creation or total matched-cell count. It must include `vesper-thorn`, `bloodroot-moon`, `crypt-iris`, and `relic-garden` on desktop and exact `390×844` mobile.
- `vesper-thorn` must complete Bloodroot Compact without Retry on both viewports under that strategy; completing on the final move is acceptable, but the bouquet may not stop at `26/27` or expose the fail state.
- Preserve the existing eight-seed optimized matrix and its current fairness envelope. Round 1 must remain 2-4 swaps, Round 2 4-7, and Round 3 3-6 for the committed seeds; do not make the slice broadly overgenerous.
- Preserve Round 1's authored opening and Black Candle Vine lesson, Round 2 Cursed Thorn teaching and CRACK/BREAK feedback, Round 3's unchanged 14 Bloodroot / 13 Sol Rot objectives, 64 tiles, eight mobile rows, save/reload, retry, keyboard/touch control, one-action ceremonies, greenhouse transactions, and Next Order flow.
...
A pass counts only when the existing three-order slice tolerates both board variance and ordinary goal-following play. A test-only scoring change, visual polish, or another progression surface does not count.
