...
- Stage continuity: the refusal snapshots the exact pre-selection copy and glowing pair, then restores that same opening, Match 3, Match 4, or armed `BLACK CANDLE` lesson rather than recalculating the board. A stale particle-clear timer was also prevented from owning invalid-feedback cleanup, so a prior match cannot prematurely erase the later refusal. Reload continues to recover the settled stage without persisting the transient message.
