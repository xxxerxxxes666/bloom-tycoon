...
- Selected weakness: the authored opening guide disappeared after the novice tapped either glowing flower. The board kept both hint highlights and generic `Swap the glowing flowers.` copy, but no longer identified the one required destination halfway through the player's first meaningful action.
- Player-visible result: the existing board guide now collapses into one restrained destination-only continuation after either hinted tile is selected. Its source/travel token retire, the remaining socket receives a stronger aged-gold diamond halo, and the existing tutorial pill says `Tap the other glowing flower.` Skip remains the sole non-tile action. Reduced motion uses the same static destination treatment.
- Input continuity: pointer and exact-mobile touch taps, either selection order, and keyboard first Enter all preserve moves `6`, objective counts, and board serialization until the destination is supplied. Keyboard focus advances to the remaining destination so the second Enter commits the authored `6 -> 5` opening match. Reload discards the transient half-selection and restores the full opening guide.
- Refusal/cancel continuity: canceled drag restores the selected or full guide as appropriate. An adjacent invalid attempt temporarily gives `NO BLOOM` sole ownership, then restores the exact selected source, destination cue, and hint pair. Nonadjacent reselection keeps the authored pair visible without committing a move. The accepted immutable repeated-refusal snapshot remains unchanged.
- Concurrent integration: origin advanced to `5ee5a54` with Hermes's continuous Round 1 guide while this pass was under verification. The destination continuation was rebased over that work so opening, Match 3, strict-four, and armed-activation stage markers/transient suppression remain authoritative; selected hinted tiles now narrow the current guide instead of hiding it.
- Browser evidence inspected: `work/first-action-selected-desktop.png`, `work/first-action-selected-mobile390.png`, and `work/first-action-guide-mobile390-reduced.png`. Desktop and exact `390x844` mobile show one selected source, one distinct destination, one instruction surface, 64 tiles/eight rows, complete first-viewport board fit, no clipping, and no horizontal overflow.
- Focused verification: the neighboring tutorial/input/feedback matrix passed `9/9`, exact-mobile real touch tap passed `1/1`, the selected-state pair/cancel/refusal matrix passed `2/2`, and the existing vertical Black Candle structural-row regression passed `5/5` after its test measured authoritative `data-y` rows instead of animation-transformed rectangle tops.
- Integrated verification: the first complete serial Chromium run passed `62/63` in `15.9m`; its only failure was the pre-existing transformed-rectangle row sampler, while all gameplay, journey, economy, ceremony, persistence, input, mobile, and runtime cases passed. After correcting that test-only sampler, its exact case passed `5/5`. A second complete run reached that correction green but the host load rose above `200`; seven late timer/teardown cases expired under system pressure and the run finished `56/63` in `1.9h`. Isolated unchanged versions of those input cases had already passed `9/9`; no product assertion required a behavior rollback.
...
- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: after an accepted match returned control, its positive feedback continued for roughly another `560ms+`. A fast adjacent no-match attempt could therefore show the new `NO BLOOM` refusal and two red X marks while the previous move still displayed `HIT`, cascade ring/vein, particles, objective flight, and order pulses over the same sockets.
- Player-visible result: committing a second adjacent tile now retires only the completed interaction's positive feedback generation before the new valid swap or `NO BLOOM` refusal speaks. A first selection, nonadjacent reselection, keyboard focus/first Enter, canceled pointer drag, and canceled touch-drag preserve the completed bouquet/order pulse through its natural lifetime. Fast input remains available under the existing timing budget; the independent `880ms` refusal generation and its immutable stage snapshot are untouched.
- Timer ownership: harvest and order-pulse cleanup timers are now tracked and generation-aware, objective-flight seal work validates both round and interaction generation, and the interaction boundary clears prior board narration plus objective/bouquet flight nodes. A stale timer cannot clear feedback created by a newer valid move.
- Deterministic regression: desktop `1280x720` pointer/tap, exact `390x844` mobile, keyboard, touch-drag, and reduced motion now cover both passive input and rapid valid-to-invalid ownership. Passive inputs preserve the same four objective/bouquet pulses without spending a move or changing board/objective state, then allow the pulses to clear on their existing timer. Refusal snapshots at `140ms`, `500ms`, and `800ms` contain exactly one `NO BLOOM | Use the glowing pair.` instruction and two refusal marks, with zero prior `HIT`, ring, vein, settle/resource/harvest effect, objective flight, bouquet seal, greenhouse flight, or order pulse. Moves remain `5`, bouquet remains `3/14`, board serialization stays unchanged, and the exact Match 3 cue/hints return after cleanup.
- Rapid valid-to-valid regression: the second accepted move presents exactly one current `HIT`, earns its own objective credit, and retains its current order pulse after the old generation's timer boundary. Focused interaction ownership coverage passed `4/4`.
- Browser evidence inspected: `work/rapid-feedback-owner-desktop.png` and `work/rapid-feedback-owner-mobile390.png` show one readable refusal, two marked flowers, 64 tiles/eight rows, complete first-viewport mobile hierarchy, no prior success narration, no clipping, and no horizontal overflow.
- Concurrent integration: origin commit `eacd6dd` added the authoritative first-action drag choreography while this pass was in progress. It was fast-forwarded beneath the feedback fix and no local or remote work was discarded.
- Focused merged verification: the first-action/passive-input/rapid-ownership/keyboard/touch/reduced-motion matrix passed `6/6`.
- Integrated verification: the complete merged serial Chromium suite passed `62/62` in `15.9m`. It retained the authored Round 1 moves `6 -> 5 -> 4 -> 3 -> 2 -> 1`, Black Candle formation/activation, Cursed Thorn feedback, atomic persistence, two exact economy cycles, ceremonies, Retry, Shuffle protection, Help/Skip, pointer, keyboard, touch/drag, and reduced motion.
...
- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: repeating an invalid adjacent swap inside the existing `880ms` refusal window canceled the first cleanup timer, then accidentally captured transient `No bloom` copy as the return state. At the authored Match 4 lesson this downgraded the visible tutorial to `Match 3 fills the bouquet.` and left the hidden cue stuck on refusal copy, even though moves and the glowing pair remained correct.
- Player-visible result: one refusal generation now owns one immutable pre-attempt cue and glowing-pair snapshot. Any repeated invalid attempt extends the same `NO BLOOM | Use the glowing pair.` response without replacing its return state; the final cleanup atomically restores the exact opening, Match 3, Match 4, or armed `BLACK CANDLE` lesson. Reload continues to restore the settled stage without persisting refusal state.
- Permanent regression: the natural desktop and exact `390x844` Round 1 journey now repeats the same invalid pair at all four tutorial stages, samples both refusal peaks, then verifies exact visible category, hidden cue, glowing-pair coordinates, 64 tiles, eight rows, and clean reload restoration. The focused natural journey passed `2/2`.
- Integrated verification: the complete serial Chromium suite passed `57/57` in `15.3m`. The authored Round 1 trace remained moves `6 -> 5 -> 4 -> 3 -> 2 -> 1`; strict-four formation, deliberate Black Candle activation, bouquet binding, `120 -> 20` restoration, first-three journeys, two-cycle economy, migration, ceremonies, atomic reloads, Cursed Thorn feedback, Retry, Shuffle protection, Help/Skip, pointer, keyboard, CDP touch-drag, and reduced motion remained green.
- Runtime/layout status: desktop, exact mobile, and reduced motion retained `509` nodes, `84` images, 64 tiles, eight complete rows, two meaningful progress bars, no dormant prototype controls, no horizontal overflow, and no visible broken images. Browser suites reported no console, page, or hard request errors.
- Static/security verification: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, changed spec and extracted playable JavaScript syntax, and `git diff --check` passed. Changed files were scanned for credential-shaped strings, secrets, trackers, analytics, and new network hooks with no findings.

## 2026-07-17 visible tutorial invalid-swap refusal

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_payoff_ceremony_contract.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: an invalid adjacent swap during the fresh Round 1 tutorial correctly spent no move and marked both tiles, but routed `No bloom` into the hidden persistent cue. The sole visible tutorial pill kept saying `Swap the glowing flowers.`, so novice error feedback contradicted the interaction they had just attempted.
- Player-visible result: the existing tutorial pill now becomes one restrained oxblood `NO BLOOM | Use the glowing pair.` instruction for the existing `880ms` refusal window. Both attempted flowers recoil away from each other with the existing red X marks, sound, and haptic feedback. Skip remains the sole non-tile action; no node, row, panel, control, mechanic, save field, or timing system was added.
- Stage continuity: the refusal snapshots the exact pre-selection copy and glowing pair, then restores that same opening, Match 3, Match 4, or armed `BLACK CANDLE` lesson rather than recalculating the board. A stale particle-clear timer was prevented from owning invalid-feedback cleanup, so a prior match cannot prematurely erase the later refusal. Repeated invalid attempts inside the same `880ms` window now retain the original stage snapshot instead of promoting transient `No bloom` copy into the return state. Reload continues to recover the settled stage without persisting the transient message.
- Browser evidence inspected: `work/invalid-feedback-desktop.png` and `work/invalid-feedback-mobile390.png` show one visible refusal pill, two directional red X marks, 64 tiles, eight rows, and no horizontal overflow. Drag captures cover desktop pointer and real exact-mobile touch input.
- Focused regression: desktop pointer/tap, exact `390x844` mobile, keyboard invalid adjacency, desktop drag, CDP touch-drag, reduced motion, opening refusal reload, Match 4 restoration, armed Black Candle restoration, and ordinary Round 2 invalid behavior passed `5/5`. The natural authored trace remained moves `6 -> 5 -> 4 -> 3 -> 2 -> 1`, strict-four credit remained exactly four Bone Stars, lane activation completed `Bouquet 14/14`, and restoration retained `120 -> 20`.
- Harness stabilization: the integrated gate exposed an existing mobile ceremony transition where the bouquet was already sealed at `14/14` before the payoff surface replaced the board. The payoff helper now accepts authoritative round completion instead of demanding another glowing pair. That exact mobile handoff passed `5/5` repeated runs without weakening ceremony assertions.
- Integrated browser verification: the complete serial Chromium suite passed `57/57` in `15.1m`. Coverage includes optimized and goal-following first-three journeys, two exact economy cycles, legacy migration, ceremonies/focus, bouquet binding, atomic reloads, Black Candle horizontal/vertical persistence and interruption, thorn feedback, Retry, Shuffle protection, Help/Skip, pointer, keyboard, touch/drag, reduced motion, and the new peak-frame refusal contract.
- Runtime/layout status: desktop, exact mobile, and reduced motion retained `509` nodes, `84` images, 64 tiles, eight complete rows, two meaningful progress bars, no dormant prototype controls, no horizontal overflow, and no visible broken images. Mobile guided swaps returned control in `326ms` and `467ms`; browser suites reported no console, page, or hard request errors.
...
- Commit/deployment: gameplay commit `2649fe9` was pushed to `origin/main`. Vercel production deployment `dpl_4xqhrw1hAChaGXwhyKbWM2thkn5B` reached `READY` and `bloom-tycoon-1003ytes9-xerxes-florals.vercel.app` was aliased to canonical `bloom-tycoon.vercel.app`; GitHub Pages published the same `main` source.
- Public-host verification: canonical Vercel and GitHub Pages each passed `4/4` across the natural desktop/mobile Round 1 journey and dedicated peak-frame refusal checks. Both retained exact stage restoration, the authored five-swap completion, 64 tiles/eight rows, one Skip action, no overflow, no broken images, and no console/page/request failures.
- Byte parity: local, canonical Vercel, and GitHub Pages playable HTML all matched SHA-256 `5f59bf96feb95734befea6ae531e15bd5dbeef322622687cab2ae8533095df4e`.

## 2026-07-17 first-action board choreography

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, and this note.
- Player-visible result: fresh unskipped Round 1 now has a board-level direct-manipulation guide for the authored opening swap. The guide renders a hollow source halo, destination halo, short connecting track, directional head, and restrained drag token from the current authoritative `.idle-hint` source tile to its destination. It does not add controls, panels, copy, assets, rules, economy, objectives, dependencies, or save fields.
- Implementation/lifecycle: geometry is derived at render time from the two authoritative hinted tiles after the board is built, using their actual rendered centers so gap/layout math stays exact. The guide is `aria-hidden`, has no focusable descendants, and uses `pointer-events:none`. It only appears before the first Round 1 move when the tutorial has not been skipped; it clears on first valid swap, Skip, post-first-move reload, payoff/round handoff, failure/retry states, and any render where the hint pair is no longer the opening pair. Pointer/touch drag hides it immediately through the existing drag-preview board state; canceled/refused input restores the same authoritative guide without spending a move. Reduced motion hides the traveling token and keeps a static connector/arrow.
- Screenshot evidence inspected: `work/first-action-guide-desktop.png`, `work/first-action-guide-mobile390.png`, `work/first-action-guide-mobile390-reduced.png`, and `work/drag-preview-desktop-ready.png`. Observed opening pair was vertical, source `{x:1,y:0}` to destination `{x:1,y:1}`; desktop and mobile screenshots show the guide centered on that pair without obscuring flower heads. Exact mobile `390x844` shows all 64 tiles and all 8 complete rows with no horizontal overflow. Reduced-motion mobile shows the same static direction with no traveling token. Active drag screenshot shows the existing drag preview owning the interaction with no double overlay.
- Verification: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, JavaScript syntax for all changed specs plus extracted inline playable code, and `git diff --check` passed. The final merged first-action/repeated-refusal/Black-Candle/payoff/feedback matrix passed `6/6`; the broader prior merged matrix passed `11/11`, and four timing-sensitive paths passed `12/12` across three repeats. After rebasing over all concurrent tutorial corrections, the complete serial Chromium suite passed `58/58` in `20.1m`, covering optimized and goal-following first-three journeys, exact economy cycles, payoff/bouquet binding, Pass 3 feedback, runtime budgets, visible and repeated invalid refusal, tutorial Skip/Replay, pre/post-move reload, pointer/touch/keyboard, invalid/cancel paths, reduced motion, Retry, Black Candle, Cursed Thorn, ceremonies, save/reload, images, overflow, and console/page/request errors.
- Integration hardening: the merged long matrices exposed pre-existing nondeterminism in three authored browser fixtures rather than gameplay failures. The payoff paths now install their known deterministic seed before navigation, the synthetic Retry state explicitly resets objective counts, and the transient Cursed Thorn recorder combines a pre-click `MutationObserver` with bounded interval samples. This prevents random cascades or a missed 16ms paint from producing false failures while preserving authoritative state and visual assertions. The concurrent `NO BLOOM` refusal—including repeated refusal within its active window—remains intact and hands cleanly back to the new first-action guide after its bounded recoil.
- Runtime/layout status: complete-suite runtime reports retained `513` nodes, `84` images, 64 tiles, 8 visible rows on desktop and exact mobile, no dormant preview/prototype nodes, no mobile plinth/log leak, no visible broken images, no horizontal overflow, and no console/page/request errors. Runtime budgets remained within thresholds: desktop animated elements `3`, mobile animated elements `3`, reduced-motion animated elements `2`; final mobile guided swaps returned control in `601ms` and `683ms`.
- Security/scope scan: changed files and changed lines were scanned for credential-shaped strings, secrets, trackers/analytics, new external URLs/network calls, private IPs, dependencies, protected-source references, and debug controls. Findings were limited to expected CSS/test words such as `token` and the existing local Playwright URL. No secrets, trackers, analytics, backend, accounts, ads, permissions, dependencies, save-schema changes, new rounds, objectives, currencies, controls, assets, or network calls were added.
- Known risk: none found in the final local browser pass.

## 2026-07-17 natural Black Candle tutorial continuity

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: a fresh unskipped player formed the persistent Black Candle Vine, but the tutorial hid the corrected persistent cue and showed only the generic sentence `Swap right to burn this row.` The special was mechanically real while its name disappeared at the exact moment the player needed to connect the armed relic to its activation.
- Player-visible result: the existing tutorial pill now reuses its icon slot as a visible, accessible `BLACK CANDLE` category whenever Round 1 has an armed relic. The exact directional action remains beside it and Skip remains the sole non-tile action. The category derives from the persisted relic state, so two reloads, an interrupted activation, and Help replay retain the named lesson; Skip reveals the already accepted persistent cue, and activation returns the icon to its ordinary decorative star.
- Mobile fit correction: the first exact `390x844` run correctly caught a small horizontal content overflow. The armed tutorial state now uses portrait-only tighter gaps, padding, and fixed type sizes while preserving the single-line pill and board position. Permanent geometry assertions verify panel/client width, copy bounds, viewport containment, no clipping, no horizontal overflow, all 64 tiles, eight complete rows, and board bottom at or above the `844px` viewport boundary.
- Focused browser verification: the natural desktop/mobile no-review journey passed `2/2`; the combined natural bouquet handoff, persistent special, cross-round cue precedence, Shuffle protection, Help/Skip, reload, interruption, and payoff matrix passed `8/8` in `1.7m`. The authored trace remains moves `6 -> 5 -> 4 -> 3 -> 2 -> 1`, strict-four credit remains exactly four Bone Stars, the row activation completes `Bouquet 14/14`, and the ceremony retains `120` earned coins before the `100`-coin restoration.
- Browser evidence inspected: `work/black-candle-desktop-formed.png` and `work/black-candle-mobile390-formed.png` show one visible `BLACK CANDLE | Swap right to burn this row.` pill with one Skip action and no duplicate cue; activated screenshots show the category removed before the bound-bouquet payoff.
- Integrated verification: the complete serial Chromium suite passed `55/55` in `15.1m`. Coverage includes optimized and goal-following first-three journeys, two exact economy cycles, legacy migration, ceremonies/focus, objective-to-bouquet binding, atomic accepted-swap reloads, Black Candle horizontal/vertical formation and activation, two-reload persistence, activation interruption, cross-round cue precedence, feedback hierarchy, Retry, Shuffle protection, Help/Skip, pointer, keyboard, touch/drag, invalid/cancel paths, reduced motion, and the new natural-session named tutorial contract.
- Runtime/layout status: desktop, exact mobile, and reduced-motion checks retained `509` nodes, `84` images, 64 tiles, eight complete rows, two meaningful progress bars, no dormant preview/prototype controls, no mobile plinth/log leak, no horizontal overflow, and no visible broken images. Mobile guided swaps returned control in `395ms` and `480ms`; browser suites reported no console, page, or hard request errors.
- Static/security verification: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, spec syntax, extracted playable JavaScript syntax, and `git diff --check` passed. Changed lines were scanned for credential-shaped strings, secrets, trackers, analytics, and new network hooks with no findings.
- Commit/deployment: gameplay commit `8f37a64` was pushed to `origin/main`. Vercel production deployment `bloom-tycoon-cy6ox6trm-xerxes-florals.vercel.app` was promoted to `bloom-tycoon.vercel.app`; GitHub Pages published the same `main` source.
- Public-host verification: the complete natural no-review Black Candle journey passed `2/2` on canonical Vercel and `2/2` on GitHub Pages across desktop `1280x720` and exact mobile `390x844`. Each run covered visible named formation, two reloads, activation interruption, Skip-to-persistent-cue handoff, Help replay, activation cleanup, bouquet binding, the sole restore action, 64 tiles/eight rows, clipping, overflow, broken images, and runtime errors.
- Byte parity: local, canonical Vercel, and GitHub Pages playable HTML all matched SHA-256 `850a35eab5091eda14ca0d2f87010c0b2c04733371ff819046528b7597db27fe`.
...
## 2026-07-17 Black Candle cue precedence

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: later-round active-play classes had more specific cue rules than the armed Black Candle state. The action text correctly said which row or column to burn, but Round 2/3 could label and color that instruction as `NEXT ORDER`, `NEXT BLOOM`, or `FINAL ORDER`, contradicting the special-piece lesson.
- Player-visible result: whenever an armed Black Candle Vine exists in focused Rounds 1-3, the existing instruction pill now has one authoritative `BLACK CANDLE` category and the restrained aged-gold/dark special palette. No cue node, mechanic, move, timing, objective, economy, save field, panel, control, or progression surface was added.
- Focused semantic matrix: desktop `1280x720` and exact mobile `390x844` cover horizontal and vertical relic guidance across Rounds 1-3. The matrix checks exact directional row/column action copy, computed pseudo-element content, computed border/text/background palette, 64 tiles/eight rows, one activation pair, full lane preview, two reloads, and an interrupted Round 2 activation that restores the exact armed pre-activation state. After activation, the ordinary Round 2 `CURSED THORN` category resumes.
- Existing special regression matrix: the authored Round 1 horizontal lesson, deliberate formation/persistence/activation, natural Round 2 vertical gravity identity, cross-round cue matrix, and later-round Retry cleanup passed `6/6` together. Screenshots `work/black-candle-cue-precedence-desktop-round2-vertical.png`, `work/black-candle-cue-precedence-mobile390-round2-vertical.png`, `work/black-candle-cue-precedence-desktop-round3-horizontal.png`, and `work/black-candle-cue-precedence-mobile390-round3-horizontal.png` show the corrected category and palette.
- Integration: origin/main advanced during verification with `d5bc057` (`feat: bind objective blooms into payoff bouquet`). The cue correction rebased over that player-visible ceremony work without a production conflict, then the complete merged gate was rerun.
- Integrated verification: the complete serial Chromium suite passed `55/55` in `14.7m`. Coverage includes optimized and goal-following first-three journeys, two exact economy cycles, legacy migration, ceremonies/focus, natural objective-to-bouquet binding, atomic accepted-swap reloads, Black Candle horizontal/vertical formation and activation, two-reload persistence, interruption recovery, feedback hierarchy, Retry, Shuffle protection, Help/Skip, pointer, keyboard, touch/drag, invalid/cancel paths, reduced motion, and the new cross-round semantic precedence matrix.
- Runtime/layout status: desktop, exact mobile, and reduced-motion checks retained `509` nodes, `84` images, 64 tiles, eight complete rows, two meaningful progress bars, no dormant preview/prototype controls, no mobile plinth/log leak, no horizontal overflow, and no visible broken images. Mobile guided swaps returned control in `386ms` and `528ms`; browser suites reported no console, page, or hard request errors.
- Static/security verification: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, `node --check scripts/verify_tutorial_progress.spec.js`, extracted inline playable JavaScript syntax, and `git diff --check` passed. Changed lines were scanned for credential-shaped strings, secrets, trackers, analytics, and new network hooks with no findings. No dependency, asset, backend, account, ad, telemetry, permission, service, currency, panel, control, round, progression, or save-schema surface was added.
- Commit/deployment: gameplay and regression coverage shipped in `efb8e1a0364f781ade07e4df1a880da57a3396fc`. Vercel production deployment `dpl_CPkM3QX3j5L75sLRyajxbidahFsG` was READY and aliased to `https://bloom-tycoon.vercel.app`; GitHub Pages workflow `29562601032` completed successfully for the same commit.
- Public-host evidence: the complete cross-round semantic matrix passed `1/1` on canonical Vercel in `52.0s` and `1/1` on GitHub Pages in `46.1s` on isolated rerun. The first Pages attempt exhausted the default 180-second whole-test cap during teardown without an assertion failure; the same unchanged matrix passed under a 300-second host allowance. Both runs cover desktop `1280x720`, exact mobile `390x844`, horizontal/vertical relics across Rounds 1-3, two reloads, activation interruption, exact computed category/palette, category restoration, 64 tiles/eight rows, focus, overflow, broken images, and runtime errors. Local, Vercel, and Pages playable SHA-256 matched `51d99610d7a3323ce370ec1f6f88463fb2193cea8d12a8c2c8f6271cf6702637`.

## 2026-07-17 atomic accepted-swap persistence

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_html_match_shapes.py`, and this note.
- Selected weakness: an accepted swap rendered and saved its spent move before the 165ms resolution delay had credited the matched flowers. Reloading or background interruption inside that window could therefore preserve the spent move while startup discarded the unresolved matched board, permanently losing the harvest.
- Player-visible result: animated resolving frames are no longer authoritative save points. The existing last fully settled snapshot remains in local storage while a swap clears, falls, refills, or cascades; the existing final render saves the complete post-swap result after control returns. Reload can recover the settled pre-swap state or a settled post-swap state, never a half-committed spent move without its objective, relic, or blocker outcome. No animation, input timing, copy, UI, save schema, economy, objective, move budget, or board rule changed.
- Deterministic interruption coverage: desktop `1280x720` and exact mobile `390x844` synchronously accepted a guided swap and queued reload before the 165ms harvest delay for three interactions: the opening Thorn Rose match, Black Candle Vine formation, and the authored Round 2 triple-thorn lesson. Each reload restored the complete unspent settled state with 64 tiles/eight rows. Replaying the interaction then credited exactly one opening harvest, exactly one armed horizontal relic with four Bone Stars, or exactly three cleared thorns; settled reloads preserved each result without duplication.
- Timing stability: the full interruption path passed `5/5` repeated runs, covering both viewports and all three interaction types on every repetition.
- Integration: origin/main advanced during verification with `feat: guide Black Candle lane activation` and its authoritative-settlement test correction. The atomic-save patch was merged over both commits, then its five-repeat interruption matrix and the complete integrated gate were rerun against the combined directional cue/lane-preview behavior.
- Integrated browser verification: the complete serial Chromium suite passed `52/52` in `13.6m`. Optimized and goal-following first-three journeys, two complete economy cycles, legacy migration, ceremonies/focus, Black Candle horizontal/vertical persistence, directional guidance and activation, simultaneous feedback, Retry, Shuffle protection, Help/Skip, pointer, keyboard, touch/drag, invalid/cancel paths, reduced motion, and the new interruption matrix all passed together. Mobile guided swaps returned control in `368ms` and `462ms`.
- Runtime/layout status: desktop, exact mobile, and reduced-motion checks retained `509` nodes, `84` images, 64 tiles, eight complete rows, two meaningful progress bars, no dormant preview/prototype controls, no mobile plinth/log leak, no horizontal overflow, and no visible broken images. Browser suites reported no console, page, or hard request errors.
- Static verification: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, `node --check scripts/verify_tutorial_progress.spec.js`, extracted inline playable JavaScript syntax, and `git diff --check` passed. Static HTML verification now requires the settled-only persistence guard.
- Security/scope: changed files and changed lines were scanned for credential-shaped strings, secrets, trackers, analytics, and new network hooks. No findings; no dependency, asset, backend, account, ad, telemetry, permission, service, currency, panel, control, round, progression, or schema surface was added.
...
- Player-visible result: Moonlit Wreath still starts with the authored Cursed Thorn lesson, but after the thorns are cleared the focused board can seed a real legal closing opportunity when target deficits are small. Single-target closeouts can now appear one move earlier in Round 2; two small Round 2 deficits can be closed by one legal swap that creates both target matches. No move budget, reward, objective, currency, save schema, UI, copy surface, asset, dependency, or completion-state rewrite was added.
- Round 3 no-worsening correction: finishing Round 2 one move earlier changed downstream gameplay RNG for `nightshade-glass`, exposing a tiny Bloodroot deficit that dragged Round 3 to 7 swaps. The existing small-deficit Round 3 legal target patch now starts at 3 moves left instead of waiting until the final fallback, keeping the committed `3-6` optimized Round 3 envelope without changing Round 3 objectives.
- Journey contract strengthened: optimized journeys now assert Round 2 `<=7` real swaps and Round 3 `<=6` real swaps. The helper now reports real legal swaps from authoritative move spend (`startMoves - movesLeft`) while keeping a separate attempt guard for stale no-spend rerenders.
- Before trace, reproduced locally before edits with `npx playwright test scripts/verify_first_three_journey.spec.js --grep "real first-three journey is fair on (desktop|mobile390) with (candle-vine|nightshade-glass)" --reporter=line`: `4/4` passed, but all four affected Round 2 paths logged `swaps:8,movesLeft:1`.
- After targeted trace, same command: `4/4` passed. Round 2 now logged:
  - desktop `candle-vine`: `swaps:7,movesLeft:2`; Round 3 `swaps:4,movesLeft:4`.
  - desktop `nightshade-glass`: `swaps:7,movesLeft:2`; Round 3 `swaps:6,movesLeft:2`.
  - exact `390x844` `candle-vine`: `swaps:7,movesLeft:2`; Round 3 `swaps:4,movesLeft:4`.
  - exact `390x844` `nightshade-glass`: `swaps:7,movesLeft:2`; Round 3 `swaps:6,movesLeft:2`.
- Full first-three matrix: `npx playwright test scripts/verify_first_three_journey.spec.js --reporter=line` passed `26/26` in `12.3m`. Optimized Round 2 range was `5-7`, Round 3 range `3-6`; goal-following desktop/mobile seeds completed; exact two-cycle economy traces remained `[[0,120,20,20],[20,170,50,50],[50,230,50,0]]` then `[[0,120,20,20],[20,170,50,50],[50,230,50]]`.
- Remaining focused Chromium matrix: `npx playwright test scripts/verify_payoff_ceremony_contract.spec.js scripts/verify_pass3_feedback.spec.js scripts/verify_runtime_performance.spec.js scripts/verify_tutorial_progress.spec.js --reporter=line` passed `21/21` in `4.4m`. Combined focused coverage for this pass: `47/47`.
- Static/syntax checks: `python3 scripts/verify_project.py`, `python3 scripts/verify_html_match_shapes.py`, `node --check scripts/verify_first_three_journey.spec.js`, extracted inline playable script syntax via `perl ... > /tmp/bloom-playable-script.js && node --check /tmp/bloom-playable-script.js`, and `git diff --check` passed. Direct `node --check playable/midnight_bloom_prototype.html` is not supported by Node for `.html`, so the inline script was checked through extraction.
- Browser/layout/error evidence: passing Playwright suites observed no console warnings/errors, page errors, failed requests, visible broken images, or horizontal overflow. Runtime checks reported desktop/mobile/reduced-motion `509` nodes, `84` images, `64` tiles, `8` complete rows, no dormant preview/prototype nodes, no mobile plinth/log leak, two meaningful bars, and no overflow. Mobile guided swaps returned control in `579ms` and `627ms`.
- Screenshots inspected: `work/first-three-mobile390-optimized-candle-vine-round2-complete.png` and `work/first-three-desktop-optimized-nightshade-glass-round2-complete.png` show Moonlit Wreath complete at `Bouquet 29/29`, exact reward/spend copy, one upgrade action, no horizontal overflow, and no broken visual state. Targeted desktop/mobile screenshots for both named seeds were captured in `work/first-three-*round2-complete.png`.
- Hermes integration acceptance: after rebasing over concurrent commits `b30ddba` (persistent Black Candle Vine) and `68b07b8` (guided-settlement test correction), the complete serial Chromium suite was rerun against the merged build and passed `49/49` in `16.9m`. The named Round 2 paths remained `7` swaps / `2` moves left on both viewports; persistent Black Candle formation/activation, later-round special use, Retry cleanup, save/reload, exact economy, reduced motion, keyboard/touch/drag, 64 tiles, 8 mobile rows, no overflow, no broken images, and no console/page/request errors all passed together.
- Known risk: the patch is intentionally deficit-window based, so several optimized Round 2 routes now close at 7 swaps rather than only the two named seeds; the full matrix kept the desired `4-7` Round 2 and `3-6` Round 3 envelopes. No live deployment was performed or checked in this pass.
- Security/scope scan: changed files were scanned for credential-shaped strings, trackers/new external network calls, private IPs, dependency changes, and protected-source references. No findings; no commit, push, deployment, cron, backend, tracker, analytics, ad, SDK, account, permission, asset, dependency, or external service change was made. I did not start a temporary server; port `4173` was already occupied and used for local browser verification.
...
## 2026-07-16 persistent Black Candle Vine special
...
- Selected weakness: the taught four-match immediately swept its lane in the same resolution wave, so players never saw, saved, or deliberately activated a familiar match-4 special piece.
- Player-visible result: a strict straight four now leaves one visibly armed black-candle relic at the moved flower's deterministic anchor. The relic preserves horizontal/vertical orientation, remains on the 8x8 board across reloads, and accepts any adjacent tap, keyboard, swipe, or drag swap. Activation spends one move, consumes the relic, and reuses the restrained row/column sweep, audio, haptic, fall, and refill language.
- Tutorial contract: after the first basic match, unskipped Round 1 guarantees one four-Bone-Star formation and one adjacent activation before bouquet completion. Formation collects exactly the four matched flowers and does not clear the lane; the next terse cue spotlights the armed relic and one adjacent tile. Skip remains the only non-tile tutorial action and exits the authored requirement without changing the six-move budget or objectives.
- Persistence/lifecycle: armed position, orientation, and source flower persist in the existing save. Two reloads preserve the exact relic and move/objective state. Activation, Shuffle, Retry, Next Order, and Play Again clear the state at the correct boundary; Help reads the current state without creating a duplicate relic. The same activation grammar was exercised in active Rounds 2 and 3.
- Browser evidence: desktop `1280x720` keyboard and exact mobile `390x844` reduced-motion touch-drag coverage verified one formed relic, 64 tiles/eight rows, no lane sweep on creation, exact `+4` Bone Star formation credit, one-move activation, exact per-flower credit for the eight flowers actually cleared from the oriented lane, no stale relic, and completion within the original six moves. The final authored Round 1 envelope is five swaps with one move remaining: moves `6 -> 5 -> 4 -> 3 -> 2 -> 1`, with formation on swap four and deliberate activation on swap five. Screenshots: `work/black-candle-formed-desktop.png`, `work/black-candle-formed-mobile390.png`, `work/black-candle-activated-desktop.png`, and `work/black-candle-activated-mobile390.png`.
- Verification: the final serial integrated Chromium gate passed `49/49` in 13.3 minutes across optimized and goal-following first-three journeys, two full economy cycles, migration, ceremonies, runtime budgets, persistent-special formation/save/reload/activation, later-round specials, Retry cleanup, tutorial/Help/Skip, tap, keyboard, swipe/drag, invalid/cancel paths, reduced motion, 64 tiles/eight rows, images, errors, and overflow.
- Scope/security: no new move budget, objective reduction, currency, inventory, booster, control, panel, round, progression surface, asset, dependency, service, tracker, analytics, account, backend, or permission was added.
...
## 2026-07-16 Round 1 Black Candle guided lesson

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_pass3_feedback.spec.js`, and this note.
- Root cause: the Round 1 Black Candle patch only ran when Bone Star progress was still `0`. Fresh-board variance could partially fill Bone Star or overfill Thorn Rose during the ordinary guided matches, so the next highlighted pair could finish `14/14` before the tutorial ever showed or executed the four-Bone-Star Black Candle move. Reload also did not recompute a mid-lesson Round 1 hint once `hasMadeValidMove` was true.
- Player-visible result: a fresh guided player now follows four authored real swaps through ordinary target matches, then sees `Match 4 makes Black Candle Vine.` with exactly two adjacent highlighted tiles. The highlighted special is a legal player swap that creates a runtime Black Candle Vine row burn. During the burn hold, the screen keeps the focused header/objective/progress, concise `Black Candle Vine! Row cleared.` cue, board, and line-relic sweep visible; the ritual log, payoff ceremony, Restore action, and board-mutating controls are hidden. After the hold, the board hides and the existing ceremony appears with exact tutorial copy `Coins restore the greenhouse.` and one `Restore Greenhouse · 100 coins` action.
- Implementation details: Round 1 now derives a lesson stage from real saved counts and moves, reseeds only the next authored tutorial move, restores staged hints after reload/refusal, and uses a dynamic Black Candle row when Thorn Rose has already overfilled. The save schema is unchanged. The completion ceremony waits on a documented `round-one-black-candle-burn-hold`: `1350ms` for normal motion, `160ms` for reduced motion. The Black Candle line sweep remains visible inside the normal-motion hold. Rounds 2-3 retain their existing move budgets and authoritative move counts; no post-completion move cushion is applied.
- Final correction: removed the hidden Round 2/3 completion rewrite that forced the move counter to one after the player had spent the final move. The honest matrix exposed two optimized zero-cushion paths: `candle-vine` completed Round 2 at `0` moves and `nightshade-glass` completed Round 3 at `0` moves. The replacement is board-path only: when a focused Round 2/3 order has all thorn requirements done, exactly two moves left, and a total target deficit of `1-4`, the existing target patcher writes one legal target swap so the player can genuinely complete with one move left. The existing Round 3 final-move board rescue remains a real legal swap fallback for completion-only strategies; saved/displayed moves are never rewritten after completion.
- Fresh no-review guided traces:
  - Desktop `1280x720`: `0/14 Moves 6` -> swap 1 `3/14 Moves 5` -> swap 2 `6/14 Moves 4` -> reload preserved moves/counts -> swap 3 `12/14 Moves 3`, cue `Make 4 Bone Stars - Black Candle Vine burns a row.`, tutorial `Match 4 makes Black Candle Vine.` -> swap 4 live burn `14/14 Moves 2`, `line-relic-hit=true`, `.board-particle.line-relic=1`, board visible/payoff hidden -> ceremony `coins=120`, one action `Restore Greenhouse · 100 coins`, payoff tokens include `x8 Thorn Rose` and `x6 Bone Star`.
  - Exact mobile `390x844`: `0/14 Moves 6` -> swap 1 `3/14 Moves 5` -> swap 2 `6/14 Moves 4` -> reload preserved moves/counts -> swap 3 `9/14 Moves 3`, same Black Candle cue/tutorial and legal four-Bone preview -> swap 4 live burn `14/14 Moves 2`, `line-relic-hit=true`, `.board-particle.line-relic=1`, all 8 rows visible/no overflow -> ceremony `coins=120`, one restore action.
- Screenshot evidence captured and inspected: `work/black-candle-desktop-cue.png`, `work/black-candle-desktop-burn.png`, `work/black-candle-desktop-ceremony.png`, `work/black-candle-mobile390-cue.png`, `work/black-candle-mobile390-burn.png`, and `work/black-candle-mobile390-ceremony.png`. Burn screenshots are viewport captures; desktop shows the board and Black Candle sweep instead of the ceremony, and exact mobile `390x844` shows all eight rows with no overflow, no ritual-log paragraph, and no Shuffle/Restore control.
- Focused burn assertions now cover: visible non-tile button list is empty; ritual log hidden; tutorial/cue name Black Candle and do not say coins; payoff hidden; board visible; `line-relic-hit` and `.board-particle.line-relic` present. Ceremony assertions then require tutorial copy `Coins restore the greenhouse.` and exactly one Restore action.
- Verification commands/results:
  - `python3 scripts/verify_project.py` passed, including HTML match-shape regression checks; `git diff --check` passed.
  - JavaScript syntax passed for `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_pass3_feedback.spec.js`, and the extracted playable inline script.
  - Focused regression passed `2/2`: `npx playwright test scripts/verify_tutorial_progress.spec.js --grep "fresh no-review guided Round 1 teaches Black Candle"`.
  - Full tutorial/drag/keyboard suite passed `9/9`: `npx playwright test scripts/verify_tutorial_progress.spec.js`.
  - Full optimized and goal-following fairness/economy matrix passed `26/26` with honest authoritative moves: `npx playwright test scripts/verify_first_three_journey.spec.js`. Optimized swap envelopes stayed bounded: Round 1 `4`, Round 2 `5-8`, Round 3 `3-6`; `candle-vine` now completes Round 2 with `1` real move left and `nightshade-glass` remains inside the Round 3 lower bound rather than relying on a display rewrite.
  - Payoff contract passed `3/3`: `npx playwright test scripts/verify_payoff_ceremony_contract.spec.js`.
  - Pass-3 feedback suite passed `3/3`: `npx playwright test scripts/verify_pass3_feedback.spec.js`.
  - Runtime suite passed `4/4`: `npx playwright test scripts/verify_runtime_performance.spec.js`; final standalone mobile guided swap latency measured `541ms` and `769ms`. A concurrent-load runtime attempt failed only the latency threshold, then passed when rerun alone.
- Browser status: passing suites observed no console warnings/errors, page errors, failed requests, visible broken images, or horizontal overflow. Active desktop/mobile retained 64 tiles and 8 rows; exact mobile kept all 8 rows visible during active play and Black Candle burn. Tutorial Skip and Replay, keyboard, tap-tap, mouse/touch drag preview, reduced motion, retry, ceremonies/economy, Round 2 Cursed Thorn, and Round 3 routes were covered by the suites above.
- Final remote integration: rebased over `2f6d8b6`, `dd695a6`, and `068fada`, preserving the replay-reinvestment cue, protected tutorial moves, and visible focused coin balance. The Black Candle hold initially consumed the coin token's 1.3-second reward pulse before the ceremony on four deterministic desktop paths; the merged behavior now starts that pulse when the hold hands off to the actionable coin ceremony. After correction, the complete merged Chromium suite passed `46/46` in `15.1m`, including the new Shuffle protection and all prior economy, fairness, tutorial, drag, ceremony, feedback, runtime, desktop, and exact-mobile checks.
- Harness correction: `scripts/verify_pass3_feedback.spec.js` now reacquires the second highlighted tile by coordinates after first-click rerender, matching the existing stale-locator-safe helpers. The first-three journey helper now waits for the intentional Round 1 burn-to-ceremony handoff before sampling the coin confirmation.
- Known issues: none found in the final local correction pass.
- Security/scope: broad changed-file scan only hit expected local Playwright URLs, CSS/test words such as `token`, and descriptive historical build-note security text. A narrower credential-shaped scan on `playable/midnight_bloom_prototype.html`, `scripts/verify_tutorial_progress.spec.js`, and `scripts/verify_pass3_feedback.spec.js` returned no matches. No assets, dependencies, services, trackers, analytics, accounts, backend, ads, monetization, cron, permissions, save-schema changes, rounds, objectives, move budgets, currencies, or private configuration were added.
...
- Player-visible result: focused active play no longer shows the old ritual-log narrator under the desktop board. The first-three slice now presents title/objective, one tutorial/cue surface, bouquet progress, one Greenhouse progress dial, the board, and contextual actions. Mobile was already hiding that footer and remains unchanged visually.
- Performance/game-feel result: `MAX_BOARD_PARTICLES` is reduced from `18` to `16` after a real exact-`390x844` runtime rerun exposed a 25-transient second-swap path against the existing `<=24` budget. The passing rerun returned control in `510ms` and `525ms`, with `12` then `14` active transients, 64 tiles, no disabled active tiles, no overflow, and no broken images.
- Runtime metrics: desktop stayed at `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 rows, 2 meaningful bars, and the ritual log hidden. Exact mobile stayed at `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 rows, hidden mobile plinth/log, and 2 meaningful bars. Reduced motion held `25` filtered / `37` shadowed / `3` animated.
- Multi-seed playtest result: `scripts/verify_first_three_journey.spec.js` passed 16/16 after the particle-cap change. All desktop and exact `390x844` mobile seeds completed Round 1 -> Round 2 -> Round 3 without zero moves; the tightest runs finished Round 1, Round 2, and Round 3 with at least `2` moves left, and the widest sampled Round 3 run finished with `6` moves left.
- Verification commands/results: `python3 scripts/verify_project.py` passed; `git diff --check` passed; `node --check` passed for all five Playwright specs; local Chromium passed `scripts/verify_runtime_performance.spec.js`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_payoff_ceremony_contract.spec.js`, and `scripts/verify_pass3_feedback.spec.js` 12/12 against `http://127.0.0.1:4262/playable/midnight_bloom_prototype.html`; `scripts/verify_first_three_journey.spec.js` passed 16/16 against the same local URL.
- Screenshot evidence inspected: `work/desktop-no-ritual-log.png`, `work/mobile390-no-ritual-log.png`, and `work/mobile390-current-delayed.png`. Visual judgment: desktop now has no bottom ritual-log strip during active play, exact mobile still shows all eight rows, and the delayed tutorial panel still says `Swap the glowing flowers.` with only `Skip`.
- Browser console/runtime status: local Chromium observed 0 console errors, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow across desktop, exact mobile, tutorial skip/replay, keyboard play, touch/click guided swaps, full Round 1 -> 2 -> 3, Cursed Thorn retry, save/reload payoff states, reduced motion, and local-QA Supreme Bloom coverage.
- Commit/push/live status: committed locally in this checkpoint. `git push origin main` failed with `Permission denied (publickey)`, and local `main` is ahead of `origin/main` by 5 commits and behind by 2. The local server returned `200` and contains `MAX_BOARD_PARTICLES = 16`; read-only Vercel and GitHub Pages checks returned `200` but do not contain that marker, so public live remains stale until this branch can be pushed from an authenticated environment.
- Known issues: no new local player-facing issue found. Branch remains ahead of and behind `origin/main`; previous push attempts in this shell failed for missing GitHub credentials.
...
- How to trigger and verify Supreme Bloom without console: on a local QA URL with `bloomReview=1`, focus the playable and press `B`; in reduced motion the static Supreme Bloom burst uses at most 16 particles and returns to 64 tiles. Normal player URLs leave this shortcut disabled, preserving Supreme Bloom rarity.
- Security/IP scan status: final changed-file and added-line scans found only expected historical build-note wording and local `segment` variable names; no credential-shaped findings or new network/tracker/account/payment hooks. This pass added no assets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, secrets, or copied protected expression.

## 2026-07-16 reduced-motion Supreme Bloom accessibility polish

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_pass3_feedback.spec.js`, and this note.
- Player-visible result: Supreme Bloom now respects reduced-motion play. In reduced motion, the rare event keeps a readable static `SUPREME BLOOM!` burst, trims JavaScript particles from `84` to `16`, skips the long charge, shortens cleanup to `650ms`, and keeps haptics suppressed through the shared reduced-motion helper. Normal-motion Supreme Bloom remains unchanged and rare.
- Recovery note: I tested the remote seven-move Bloodroot Compact authored-opening idea before this polish. It still exhausted `thorn-crack-matrix` Round 3 under the hardened journey helper and made `amber-sun-matrix` complete Round 3 in one swap, so I reverted that experiment and preserved the previously verified eight-move Bloodroot Compact tuning.
- Verification commands/results: extracted playable JavaScript passed `node --check`; all five Playwright specs passed `node --check`; `python3 scripts/verify_project.py` passed; `git diff --check` passed; local Chromium passed `scripts/verify_runtime_performance.spec.js`, `scripts/verify_tutorial_progress.spec.js`, `scripts/verify_payoff_ceremony_contract.spec.js`, and `scripts/verify_pass3_feedback.spec.js` 12/12 against `http://127.0.0.1:4261/playable/midnight_bloom_prototype.html`.
- Multi-seed playtest result: `scripts/verify_first_three_journey.spec.js` passed 16/16 after the final CSS fix. All desktop and exact `390x844` mobile seeds completed Round 1 -> Round 2 -> Round 3 without zero moves and reached `Restore Greenhouse`, `Next Order -> Moonlit Wreath`, `Upgrade Greenhouse`, `Next Order -> Bloodroot Compact`, `Raise Conservatory`, and `Play Again -> First Bouquet`.
- Runtime metrics: desktop stayed at `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 rows, 2 meaningful bars. Exact mobile stayed at `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 rows, hidden mobile plinth/log, 2 meaningful bars. Reduced motion held `25` filtered / `37` shadowed / `3` animated. Mobile guided swaps returned control in `442ms` and `701ms`, with `12` then `22` active transients.
- Screenshot evidence inspected: `work/pass3-mobile390-reduced-supreme.png`, `work/pass3-mobile390-round3-raised.png`, and `work/first-three-mobile390-rose-compact-matrix-replay.png`. Visual judgment: reduced-motion Supreme Bloom is now static and readable, exact mobile still shows eight board rows, and the Round 3 ceremony remains one trophy, one greenhouse before/after, concise coins, and one dominant replay action.
- Browser console/runtime status: final local Chromium suites observed 0 console errors/warnings, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow across desktop, exact mobile, tutorial skip/replay, keyboard play, touch/click guided swaps, full Round 1 -> 2 -> 3, Cursed Thorn retry, save/reload payoff states, reduced motion, and local-QA Supreme Bloom.
- Commit/push/live status: pending commit at note time. Local server returned `200`; public Vercel and GitHub Pages are expected to remain stale until this local stack can be pushed from an authenticated environment.
- Known issues: no new local player-facing issue found. Branch remains ahead of and behind `origin/main` because `origin/main` contains an alternate fairness checkpoint and this shell still lacks push credentials.
- How to trigger and verify L/T/cross matches without console: on a local QA URL with `bloomReview=1`, focus the playable and press `M` until `Witch's Cross!`, `Night Garden L-Bloom!`, or `Twin Stem Bloom!` appears. Normal player URLs leave this shortcut disabled; real L/T/cross resolution remains covered by the focused feedback suite.
- How to trigger and verify Supreme Bloom without console: on a local QA URL with `bloomReview=1`, focus the playable and press `B`; in reduced motion the static Supreme Bloom burst uses at most 16 particles and returns to 64 tiles. Normal player URLs leave this shortcut disabled, preserving Supreme Bloom rarity.
- Security/IP scan status: final changed-file and added-line scans found only expected local Playwright/public status URLs and descriptive build-note text; no credential-shaped findings. This pass added no assets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, secrets, or copied protected expression.

## 2026-07-16 active feedback latency and particle cap
...
- Player-visible result: active match feedback is lighter and less likely to build up during fast phone play. Objective flights, greenhouse intake motes, and bouquet bind seals keep the same readable reward path, but use smaller/lighter transient effects. Board particle effects now route through one priority-aware cap so sparks, petals, coin pops, and shards trim before high-signal events like central HIT, Cursed Thorn shock, Black Candle sweep, and order seals.
- Performance/game-feel result: `MAX_BOARD_PARTICLES` is reduced from `24` to `18`, and the focused mobile browser spec now requires total active transient effects after the second guided swap to stay at `<=24` instead of `<=32`. Exact `390x844` guided swaps returned control in `414ms` and `670ms`, with `12` then `22` active transients, 64 tiles, no disabled tiles, no overflow, and no broken images.
- Runtime metrics: desktop remained at `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 rows, 2 meaningful bars. Exact mobile remained at `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 rows, hidden mobile plinth/log, 2 meaningful bars. Reduced motion held `25` filtered / `37` shadowed / `3` animated.
- Verification commands/results: extracted playable JavaScript passed `node --check`; `python3 scripts/verify_project.py` passed; `git diff --check` passed; local Chromium passed `scripts/verify_runtime_performance.spec.js` 4/4, `scripts/verify_tutorial_progress.spec.js` + `scripts/verify_first_three_journey.spec.js` 19/19, and `scripts/verify_payoff_ceremony_contract.spec.js` + `scripts/verify_pass3_feedback.spec.js` 4/4 against `http://127.0.0.1:4260/playable/midnight_bloom_prototype.html`.
- Multi-seed playtest result: all 16 no-review desktop/mobile journeys completed Round 1 -> Round 2 -> Round 3 without reaching zero moves. The tightest sampled runs still finished every order with at least 2 moves left and showed the expected restore, next-order, upgrade, conservatory, and replay actions.
- Screenshot evidence inspected: `work/first-three-mobile390-thorn-crack-matrix-replay.png` and `work/first-three-mobile390-rose-compact-matrix-round3-complete.png`. Visual judgment: exact mobile still shows eight complete board rows, readable tutorial spotlighting, one bouquet bar plus one Greenhouse bar, and the Round 3 ceremony remains one trophy, one greenhouse before/after, concise coins, and one dominant action.
- Browser console/runtime status: final local Chromium suites observed 0 console errors/warnings, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow across desktop, exact mobile, touch/click guided swaps, keyboard play, tutorial skip/replay, full Round 1 -> 2 -> 3, Cursed Thorn retry, save/reload payoff states, reduced motion, and local-QA Supreme Bloom review hook coverage.
- Commit/push/live status: committed locally as `Cap active feedback particles`. `git push origin main` failed with `Permission denied (publickey)`, and direct HTTPS push failed with `could not read Username for 'https://github.com': No such device or address`; local `main` is ahead of `origin/main` by 3 commits. Read-only checks returned `200` for local, Vercel, and GitHub Pages. Local playable SHA-256 is `1404ad9c6635f6603cea8fd1e64e3b5ee093dce29b7cbf26a8892e37bdd0f2a6` and contains `const MAX_BOARD_PARTICLES = 18`; Vercel and GitHub Pages both served stale SHA-256 `1d4557dc7656b03c5a6203b4e1c75cb82c4614eefc0f7c1c5be2e71624b134c7` without the new cap.
- Known issues: no new local player-facing issue found. Public live remains stale until this local checkpoint stack can be pushed and deployed from an authenticated environment.
- How to trigger and verify L/T/cross matches without console: on a local QA URL with `bloomReview=1`, focus the playable and press `M` until `Witch's Cross!`, `Night Garden L-Bloom!`, or `Twin Stem Bloom!` appears. Normal player URLs leave this shortcut disabled; real L/T/cross resolution remains covered by the focused feedback suite.
...
- Security/IP scan status: changed files and added lines were scanned for private-key headers, provider token prefixes, JWT-like strings, credential assignments, `.env`, external URLs, trackers/analytics/payment/ad/backend/account hooks, dependency or permission changes, and copied protected-source references. Only expected local Playwright URLs and descriptive build-note text matched broad patterns; no findings. This pass added no assets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, secrets, or copied protected expression.
...
## 2026-07-15 tutorial replay and ceremony latency polish

- Files changed: `playable/midnight_bloom_prototype.html`, `scripts/verify_first_three_journey.spec.js`, `scripts/verify_payoff_ceremony_contract.spec.js`, and this note.
- Player-visible result: the active-board tutorial replay entry is now a compact `?` control with `aria-label="Replay Tutorial"`, reducing active-play text clutter while keeping the tutorial skippable and replayable. Greenhouse ceremony art now loads deterministically, eliminating the keyboard save/reload `first_greenhouse_restored.jpg net::ERR_ABORTED` path found during baseline QA.
- Performance/game-feel result: bouquet binding seals resolve sooner and no longer animate `filter`, preventing stacked seal remnants after rapid mobile swaps while preserving the bouquet-to-order confirmation.
- Verifier hardening: the first-three journey bot now clicks the live tile center with real mouse input after selecting a pair, avoiding a Playwright locator visibility race without changing game logic.
- Verification commands/results: `python3 scripts/verify_project.py` passed; `git diff --check` passed; `node --check` passed for all five Playwright specs; local Chromium passed 13/13 against `http://127.0.0.1:4252/playable/midnight_bloom_prototype.html` using temp-installed `@playwright/test 1.61.1`.
- Browser coverage: desktop `1280x720`, exact mobile `390x844`, touch/click guided swaps, keyboard board play and reload, tutorial skip/replay, full Round 1 -> 2 -> 3, Cursed Thorn fail/retry, save/reload payoff states, reduced motion, local-QA Supreme Bloom review hook, 64-tile/eight-row integrity, no horizontal overflow, no visible broken images, and no console/page/request failures.
- Runtime metrics: desktop `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 rows, 2 meaningful bars. Exact mobile `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 rows, hidden mobile plinth/log, 2 meaningful bars. Reduced motion held `25` filtered / `37` shadowed / `3` animated.
- Latency metrics: exact mobile guided swaps returned control in `441ms` and `631ms` in the final full suite, with 64 tiles, no disabled active tiles, no overflow, no broken images, and particle buildup at `29`.
- Real no-review playtest result: seeded Chromium desktop and exact mobile both completed Round 1 -> 2 -> 3 in 3, 6, and 5 swaps. Each round ended with 3, 2, and 2 moves left respectively, then showed the expected `Restore Greenhouse`, `Next Order -> Moonlit Wreath`, `Upgrade Greenhouse`, `Next Order -> Bloodroot Compact`, `Raise Conservatory`, and `Play Again -> First Bouquet` actions.
- Screenshot evidence inspected: `work/first-three-mobile390-replay.png`, `work/first-three-desktop-replay.png`, `work/pass2-mobile390-round1-restored.png`, and `work/pass2-mobile390-round3-raised.png`. Visual judgment: eight rows remain visible on exact mobile, the `?` replay control is unobtrusive, greenhouse ceremony art is intact, and no overlap or broken art was visible.
- Browser console/runtime status: final local Chromium suite observed 0 console errors/warnings, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow.
- Commit/push/live status: committed locally as this checkpoint. `git push origin main` failed with `Permission denied (publickey)`, and HTTPS push failed with `could not read Username for 'https://github.com': No such device or address`. Read-only GitHub Pages and Vercel checks returned `200` but still serve the stale `Help` button, lazy ceremony greenhouse images, and old `1180ms` bouquet seal.
- Known issues: no new local player-facing issue found. Local `main` is 1 commit ahead of `origin/main`; public GitHub Pages and Vercel remain stale until push credentials are available.
- How to trigger and verify L/T/cross matches without console: on a local QA URL with `bloomReview=1`, focus the playable and press `M` until `Witch's Cross!`, `Night Garden L-Bloom!`, or `Twin Stem Bloom!` appears. Normal player URLs leave this shortcut disabled; real L/T/cross resolution remains covered by the focused feedback suite.
- How to trigger and verify Supreme Bloom without console: on a local QA URL with `bloomReview=1`, focus the playable and press `B`; `SUPREME BLOOM!` appears and returns to 64 tiles after the charge. Normal player URLs leave this shortcut disabled, preserving Supreme Bloom rarity.
- Security/IP scan status: changed files and added lines were scanned for private keys, provider tokens, JWT-like strings, credential assignments, `.env`, external URLs, trackers/analytics/payment/ad/backend/account hooks, dependency or permission changes, and copied protected-source references. Only expected local test URL references and ordinary variable names matched broad-file patterns; no findings. This pass added no secrets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, or copied protected expression.
...
- Player-visible result: the first three orders now use tighter but still fair move budgets: First Bouquet `Moves 6`, Moonlit Wreath `Moves 8`, and Bloodroot Compact `Moves 7`. Targets, coins, greenhouse costs, ceremonies, tutorial prompts, tile art, rewards, and replay flow are unchanged.
- Real no-review playtest result: seeded Chromium desktop `1280x720` and exact mobile `390x844` both completed Round 1 -> 2 -> 3 without review hooks in 3, 6, and 5 swaps. Round 1 finished with 3 moves left; Round 2 and Round 3 each finished with 2 moves left; the flow then showed `Restore Greenhouse`, `Next Order -> Moonlit Wreath`, `Upgrade Greenhouse`, `Next Order -> Bloodroot Compact`, `Raise Conservatory`, and `Play Again -> First Bouquet`.
- Regression coverage: added `scripts/verify_first_three_journey.spec.js`, a seeded no-review player-bot route that reads the board DOM, chooses legal target-advancing swaps, verifies desktop and exact mobile layout, captures journey screenshots, and asserts the first-three route is neither failing nor overgenerous.
- Runtime cleanup: active play keeps the active greenhouse image on the already-loaded withered art instead of swapping hidden Moonlit/Bloodroot stage images; ceremony greenhouse images lazy-load and the keyboard reload test waits for images before deliberate reload, eliminating test-induced aborted greenhouse requests.
- Verification commands/results: `python3 scripts/verify_project.py` passed; `git diff --check` passed; `node --check` passed for all five Playwright specs; local Chromium passed 13/13 against `http://127.0.0.1:4251/playable/midnight_bloom_prototype.html` using cached Playwright.
- Browser coverage: desktop `1280x720`, exact mobile `390x844`, touch/click guided swaps, keyboard board play and reload, tutorial skip/replay, full Round 1 -> 2 -> 3, Cursed Thorn fail/retry, save/reload payoff states, reduced motion, local-QA Supreme Bloom review hook, 64-tile/eight-row integrity, no horizontal overflow, no visible broken images, and no console/page/request failures.
- Runtime metrics: desktop `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 rows, 2 meaningful bars. Exact mobile `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 rows, hidden mobile plinth/log, 2 meaningful bars. Reduced motion held `25` filtered / `37` shadowed / `3` animated.
- Latency metrics: exact mobile guided swaps returned control in `421ms` and `500ms` in the final full suite, with 64 tiles, no disabled active tiles, no overflow, no broken images, and particle buildup at `16`.
- Screenshot evidence inspected: `work/first-three-mobile390-replay.png` and `work/first-three-desktop-replay.png`. Visual judgment: the board remains the dominant surface, eight rows are visible, the two progress bars are readable, and no overlap or broken art was visible.
- Browser console/runtime status: final full Chromium suite observed 0 console errors/warnings, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow.
- Commit/push/live status: committed locally as `Tune first-three journey fairness`. `git push origin main` failed with `Permission denied (publickey)`, and direct HTTPS push failed with `could not read Username for 'https://github.com'`; public GitHub Pages and Vercel were read-only checked and still serve the older `Moves 12/14/14` build.
- Known issues: local `main` remains 9 commits ahead of `origin/main` until push credentials are available. No new local player-facing issue found.
- How to trigger and verify L/T/cross matches without console: on a local QA URL with `bloomReview=1`, focus the playable and press `M` until `Witch's Cross!`, `Night Garden L-Bloom!`, or `Twin Stem Bloom!` appears. Normal player URLs leave this shortcut disabled; real L/T/cross resolution remains covered by the focused feedback suite.
- How to trigger and verify Supreme Bloom without console: on a local QA URL with `bloomReview=1`, focus the playable and press `B`; `SUPREME BLOOM!` appears and returns to 64 tiles after the charge. Normal player URLs leave this shortcut disabled, preserving Supreme Bloom rarity.
- Security/IP scan status: changed files were scanned for private keys, provider tokens, JWT-like strings, credential assignments, `.env`, external URLs, trackers/analytics/payment/ad/backend/account hooks, dependency or permission changes, and copied protected-source references. Only expected local test URL references and existing asset paths matched; no findings. This pass added no secrets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, or copied protected expression.
...
- Player-visible result: the live six-flower set is replaced with a more coherent original late-1990s dark-fantasy inventory set. Sol Rot is now an abstract cracked sun/seed medallion instead of a face; Bone Star, Nightshade, Bloodroot, Amber Seed, and Thorn Rose keep compact readable silhouettes with darker oxidized edges and upper-left candlelit highlights.
- Asset workflow: used the built-in `image_gen` tool with the user-provided UI reference and prior spritesheet as style/quality references only, copied the selected project-bound spritesheet into `assets/tiles/generated/`, stripped generated PNG metadata/provenance chunks, then regenerated transparent 96px and 48px crops locally. No copied Diablo/Blizzard assets, fonts, frames, icons, monsters, or protected UI expression were added.
- Runtime cleanup: removed two nonessential greenhouse image `prefetch` hints after the keyboard save/reload test caught an aborted speculative request. During focused active play, the large greenhouse CSS background now stays on the already-loaded withered image while the visible active-stage element remains available for greenhouse intake feedback; ceremony art still loads when used. This reduced fresh runtime DOM nodes from `467` to `465`.
- Verification commands/results: `python3 scripts/verify_project.py` passed; `git diff --check` passed; `node --check` passed for all four Playwright specs; local Chromium passed 11/11 against `http://127.0.0.1:4245/playable/midnight_bloom_prototype.html` using `@playwright/test 1.61.1`.
- Browser coverage: desktop `1280x720`, exact mobile `390x844`, touch/click guided swaps, keyboard board play, tutorial skip/replay, full Round 1 -> 2 -> 3, Cursed Thorn fail/retry, save/reload payoff states, reduced motion, local-QA Supreme Bloom review hook, 64-tile/eight-row integrity, no horizontal overflow, no visible broken images, and no console/page/request failures.
- Runtime metrics: desktop `465` nodes, `80` images, `26` filtered, `36` shadowed, `3` animated, 64 tiles, 8 visible board rows, 0 dormant preview nodes, 0 prototype scaffold nodes. Exact mobile `465` nodes, `80` images, `27` filtered, `39` shadowed, `3` animated, 64 tiles, 8 visible board rows, one visible bouquet progress bar, and `activeGreenhouseStage` as the intake target. Reduced motion held `25` filtered / `37` shadowed / `3` animated.
- Latency metrics: exact mobile guided swaps returned control in `418ms` and `674ms`, with 64 tiles, no disabled active tiles after settle, no overflow, no broken images, and particle buildup within the existing budget.
- No-review playtest result: a real local Chromium path completed Round 1 in 3 moves, Round 2 in 6 moves, and Round 3 in 7 moves, ending on `Raise Conservatory · 180 coins`. The probe recorded 64 tiles, no overflow, no visible broken images, no console messages, no page errors, and no failed requests.
- Screenshot evidence inspected: `work/art-pass-six-flower/six-flower-contact.png`, `desktop-fresh.png`, `mobile390-fresh.png`, `mobile390-after-real-swap.png`, `desktop-round1-ceremony.png`, and `desktop-round3-ceremony.png`. Visual judgment: all six icons remain readable in board sockets at exact mobile size; the set is more original and less polished-fantasy than the previous sheet.
- Browser console/runtime status: final local Chromium suite and no-review probe observed 0 console errors/warnings, 0 page errors, 0 failed requests, 0 visible broken images, and no horizontal overflow.
- Commit/push/live status: pending commit at note time. Public GitHub Pages/Vercel were not yet updated from this environment; local `main` remains ahead of and behind `origin/main` before push reconciliation.
- Known issues: public live may remain stale until `origin/main` accepts the local checkpoint stack. No new local player-facing issue found.
- How to trigger and verify L/T/cross matches without console: on a local QA URL with `bloomReview=1`, focus the playable and press `M` until `Witch's Cross!`, `Night Garden L-Bloom!`, or `Twin Stem Bloom!` appears. Normal player URLs leave this shortcut disabled; real L/T/cross resolution remains covered by the focused feedback suite.
- How to trigger and verify Supreme Bloom without console: on a local QA URL with `bloomReview=1`, focus the playable and press `B`; `SUPREME BLOOM!` appears and returns to 64 tiles after the charge. Normal player URLs leave this shortcut disabled, preserving Supreme Bloom rarity.
- Security/IP scan status: changed files were scanned with `strings`/`rg` for private keys, provider tokens, JWT-like strings, credential assignments, `.env`, external URLs, trackers/analytics/payment/ad/backend/account hooks, dependency or permission changes, and copied protected-source references. Initial generated spritesheet metadata contained C2PA certificate/OCSP URLs; those ancillary chunks were stripped and the final scan had no findings. This pass added no secrets, dependencies, SDKs, trackers, analytics, backend, accounts, payments, ads, broad permissions, or copied protected expression.
