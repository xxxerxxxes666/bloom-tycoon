...
async function clickGuidedSwap(page, options = {}) {
  ...
  await expect(hints).toHaveCount(2, { timeout: 12000 });
...
function assertBlackCandleActivationFrames(frames, testCase, persisted, forecast) {
  ...
  for (const frame of active) {
    ...
    if (frame.roundComplete) {
      expect(
        frame.visibleInstructionText,
        `${testCase.label} completed payoff suppresses the shared activation narrator`
      ).toEqual([]);
    } else {
      expect(
...
for (const viewport of [
  ...
  test(`fresh no-review guided Round 1 teaches Black Candle before payoff on ${viewport.label}`, async ({ page }) => {
    ...
    expect(activated.visibleInstructionSurfaces).toBe(0);
    ...
    expect(ceremony.visibleInstructionSurfaces).toBe(0);
  ...
  for (const testCase of cases) {
    ...
    try {
      ...
      for (let remaining = 0; remaining < 4 && !(await activeState(page)).roundComplete; remaining += 1) {
        const beforeGuide = await guidedRoundOneState(page, `${testCase.label} closing guide ${remaining + 1} before`);
        await expect(
          page.locator(".tile.idle-hint"),
          `${testCase.label} closing guide ${remaining + 1} returns from ${beforeGuide.tutorial} at moves ${beforeGuide.moves}`
      ...
      await expect(page.locator("#roundOneRestoration")).toBeVisible({ timeout: 15000 });
      await expect(page.locator("#restoreGreenhouseBtn")).toBeVisible({ timeout: 15000 });
      expect(consoleErrors).toEqual([]);
      expect(pageErrors).toEqual([]);
      expect(failedRequests).toEqual([]);
    } finally {
...
test("repeated Round 1 exploration still closes through the displayed six-move contract", async ({ browser }) => {
  test.setTimeout(420000);
  const cases = [
    { label: "desktop-pointer", viewport: { width: 1280, height: 720 }, input: "pointer" },
    { label: "mobile390-touch", viewport: { width: 390, height: 844 }, input: "touch", mobile: true },
  ...
  const openingOffOrderPair = [{ x: 3, y: 2 }, { x: 4, y: 2 }];
  const repeatedOffOrderPair = [{ x: 6, y: 1 }, { x: 6, y: 2 }];
  const targetIds = [5, 1];
  ...
  const activatePair = async (page, pair, input, { completes = false } = {}) => {
    ...
    await page.waitForFunction(({ key, before, completes }) => {
      const saved = JSON.parse(localStorage.getItem(key) || "{}");
      return completes
        ? Boolean(saved.roundComplete)
        : Number(saved.moves) === Number(before) - 1
          && Array.from(document.querySelectorAll(".tile")).every((node) => !node.disabled);
    }, { key: SAVE_KEY, before, completes }, { timeout: 12000 });
    if (!completes) {
      await waitForSettledBoard(page);
    }
  };

  const assertNoFalseFillPromise = async (page, label) => {
    const report = await page.evaluate((key) => {
      const tutorial = document.querySelector("#tutorialCopy")?.textContent.trim() || "";
      if (tutorial !== "Match 3 fills the bouquet.") {
        return { tutorial, closes: null };
      }
      const saved = JSON.parse(localStorage.getItem(key) || "{}");
      const next = saved.board.map((row) => row.slice());
      const hints = Array.from(document.querySelectorAll(".tile.idle-hint")).map((node) => ({
        x: Number(node.dataset.x),
        y: Number(node.dataset.y)
      }));
      if (hints.length !== 2) {
        return { tutorial, closes: false };
      }
      const [a, b] = hints;
      [next[a.y][a.x], next[b.y][b.x]] = [next[b.y][b.x], next[a.y][a.x]];
      const gained = new Map();
      const addRun = (flowerId, cells) => {
        if (!gained.has(flowerId)) gained.set(flowerId, new Set());
        cells.forEach(([x, y]) => gained.get(flowerId).add(`${x},${y}`));
      };
      for (let y = 0; y < 8; y += 1) {
        for (let start = 0, x = 1; x <= 8; x += 1) {
          if (x === 8 || next[y][x] !== next[y][start]) {
            if (x - start >= 3) {
              addRun(next[y][start], Array.from({ length: x - start }, (_, index) => [start + index, y]));
            }
            start = x;
          }
        }
      ...
      for (let x = 0; x < 8; x += 1) {
        for (let start = 0, y = 1; y <= 8; y += 1) {
          if (y === 8 || next[y][x] !== next[start][x]) {
            if (y - start >= 3) {
              addRun(next[start][x], Array.from({ length: y - start }, (_, index) => [x, start + index]));
            }
            start = y;
          }
        }
      ...
      return {
        tutorial,
        closes: (saved.counts[5] || 0) + (gained.get(5)?.size || 0) >= 8
          && (saved.counts[1] || 0) + (gained.get(1)?.size || 0) >= 6
      };
    }, SAVE_KEY);
    if (report.tutorial === "Match 3 fills the bouquet.") {
      expect(report.closes, `${label} fill promise matches the guided swap's actual target yield`).toBe(true);
    }
  };
  ...
  for (const testCase of cases) {
    ...
    try {
      await seedDeterministicMath(page, "fairness-seed-9");
      await openFreshNoReview(page, `round-one-closing-fairness-${testCase.label}`);

      await activatePair(page, openingOffOrderPair, testCase.input);
      let correction = await guidedRoundOneState(page, `${testCase.label} opening correction`);
      expect(correction.moves).toBe(5);
      expect(targetIds.map((flowerId) => correction.counts[flowerId])).toEqual([0, 0]);
      expect(correction.tutorial).toBe("Match the order flowers.");
      expect(correction.hints).toEqual([]);
      await assertNoFalseFillPromise(page, `${testCase.label} opening correction`);

      await expect(
        page.locator(".tile.idle-hint"),
        `${testCase.label} opening correction returns guidance`
      ).toHaveCount(2, { timeout: 8500 });
      await activatePair(page, await hintedPair(page), testCase.input);
      const firstTargetMove = await guidedRoundOneState(page, `${testCase.label} first recovered target`);
      expect(firstTargetMove.moves).toBe(4);
      expect(firstTargetMove.counts[5]).toBe(3);
      await assertNoFalseFillPromise(page, `${testCase.label} first recovered target`);

      for (let exploration = 0; exploration < 2; exploration += 1) {
        const before = await guidedRoundOneState(page, `${testCase.label} exploration ${exploration + 1} before`);
        await activatePair(page, repeatedOffOrderPair, testCase.input);
        correction = await guidedRoundOneState(page, `${testCase.label} exploration ${exploration + 1} correction`);
        expect(correction.moves).toBe(3 - exploration);
        expect(
          targetIds.map((flowerId) => correction.counts[flowerId] - before.counts[flowerId]),
          `${testCase.label} exploration ${exploration + 1} earns zero order flowers`
        ).toEqual([0, 0]);
        expect(correction.tutorial).toBe("Match the order flowers.");
        expect(correction.hints).toEqual([]);
        expect(correction.visibleInstructionSurfaces).toBe(1);
        expect(correction.visibleButtons).toEqual(["Skip"]);
        expect(correction.tiles).toBe(64);
        expect(correction.rows).toBe(8);
        expect(correction.overflowX).toBe(false);
        expect(correction.brokenImages).toEqual([]);
        await assertNoFalseFillPromise(page, `${testCase.label} exploration ${exploration + 1}`);
        const focusBeforeGuide = await page.evaluate(() => document.activeElement?.id || "");
        await expect(
          page.locator(".tile.idle-hint"),
          `${testCase.label} exploration ${exploration + 1} returns guidance`
        ).toHaveCount(2, { timeout: 8500 });
        expect(
          await page.evaluate(() => document.activeElement?.id || ""),
          `${testCase.label} correction guide does not steal focus`
        ).toBe(focusBeforeGuide);
        if (exploration === 0) {
          expect((await guidedRoundOneState(page, `${testCase.label} first exploration guide`)).tutorial)
            .toBe("Match Thorn Rose.");
        }
      }
      ...
      expect(guideReport.pointerEvents).toBe("none");
      expect(guideReport.tiles).toBe(64);
      expect(guideReport.rows).toBe(8);
      expect(guideReport.overflowX).toBe(false);
      expect(guideReport.brokenImages).toEqual([]);
      if (testCase.mobile) {
        expect(guideReport.completeRows).toBe(8);
        expect(guideReport.boardBottom).toBeLessThanOrEqual(844);
      }
      await assertNoFalseFillPromise(page, `${testCase.label} two-move close`);

      const beforeFormation = closingGuide.counts.slice();
      await activatePair(page, closingGuide.hints, testCase.input);
      const formed = await guidedRoundOneState(page, `${testCase.label} closing Black Candle formed`);
      expect(formed.moves).toBe(1);
      expect(formed.counts[1] - beforeFormation[1]).toBe(4);
      ...
      await page.reload({ waitUntil: "networkidle" });
      await expect(page.locator(".tile.idle-hint")).toHaveCount(2, { timeout: 4000 });
      const reloaded = await guidedRoundOneState(page, `${testCase.label} closing lane reload`);
      ...
      expect(reloaded.roundComplete).toBe(false);
      await expect(page.locator(".impact-sigil, .objective-flight, .order-pulse")).toHaveCount(0);

      await page.evaluate(() => {
        const tutorial = document.querySelector("#tutorialCopy");
        window.__roundOneFairnessNarration = [tutorial?.textContent.trim() || ""];
        window.__roundOneFairnessNarrationObserver = new MutationObserver(() => {
          const text = tutorial?.textContent.trim() || "";
          if (text && !window.__roundOneFairnessNarration.includes(text)) {
            window.__roundOneFairnessNarration.push(text);
          }
        });
        window.__roundOneFairnessNarrationObserver.observe(tutorial, {
          childList: true,
          characterData: true,
          subtree: true
        });
      ...
      await expect(page.locator("#roundOneRestoration")).toBeVisible({ timeout: 12000 });
      await expect(page.locator("#tutorialPanel")).toBeHidden({ timeout: 12000 });
      await expect(page.locator("#tutorialCopy")).toHaveText("");
      const closingNarration = await page.evaluate(() => {
        window.__roundOneFairnessNarrationObserver?.disconnect();
        return window.__roundOneFairnessNarration || [];
      });
      expect(closingNarration.join(" · ")).not.toMatch(/Next Order|Restore Greenhouse/i);
      const completed = await guidedRoundOneState(page, `${testCase.label} closing activation complete`);
      const completedDataRows = await page.locator(".tile").evaluateAll((tiles) => (
        new Set(tiles.map((tile) => Number(tile.dataset.y))).size
      ));
      ...
      expect(completed.bouquet).toBe("Bouquet Complete · 14/14");
      expect(completed.tutorial).toBe("");
      expect(completed.visibleInstructionSurfaces).toBe(0);
      expect(completed.payoffVisible).toBe(true);
      expect(completed.tiles).toBe(64);
      expect(completedDataRows).toBe(8);
      expect(completed.overflowX).toBe(false);
      expect(completed.brokenImages).toEqual([]);
      expect(consoleErrors).toEqual([]);
      expect(pageErrors).toEqual([]);
...
test.describe("Escape cancels a keyboard flower selection without losing board guidance", () => {
  test.describe.configure({ mode: "parallel" });
  const cases = [
    {
      label: "desktop-enter-source-full",
      viewport: { width: 1280, height: 720 },
      activationKey: "Enter",
      endpointIndex: 0,
      reducedMotion: "no-preference"
    ...
    {
      label: "desktop-space-destination-reduced",
      viewport: { width: 1280, height: 720 },
      activationKey: "Space",
      endpointIndex: 1,
      reducedMotion: "reduce"
    ...
    {
      label: "mobile-enter-destination-full",
      viewport: { width: 390, height: 844 },
      activationKey: "Enter",
      endpointIndex: 1,
      reducedMotion: "no-preference"
    ...
    {
      label: "mobile-space-source-reduced",
      viewport: { width: 390, height: 844 },
      activationKey: "Space",
      endpointIndex: 0,
      reducedMotion: "reduce"
  ...
  const cancelAndCommit = async (page, testCase, stage) => {
    ...
    const selectedTile = page.locator(`#tile-${selectedCell.x}-${selectedCell.y}`);
    ...
    expect(selected.selected, `${testCase.label} ${stage} owns one source`).toEqual([
      `tile-${selectedCell.x}-${selectedCell.y}`
    ]);

    await page.keyboard.press("Escape");
    const canceled = await snapshot(page);
    expect(canceled.save, `${testCase.label} ${stage} Escape has no save drift`).toBe(before.save);
    expect(canceled.moves, `${testCase.label} ${stage} Escape spends no move`).toBe(before.moves);
    expect(canceled.counts, `${testCase.label} ${stage} Escape preserves counts`).toEqual(before.counts);
    expect(canceled.boardState, `${testCase.label} ${stage} Escape preserves board`).toBe(before.boardState);
    expect(canceled.armedLineRelic, `${testCase.label} ${stage} Escape preserves relic`).toEqual(before.armedLineRelic);
    expect(canceled.selected, `${testCase.label} ${stage} Escape clears selection`).toEqual([]);
    expect(canceled.hints, `${testCase.label} ${stage} Escape restores exact pair`).toEqual(before.hints);
    expect(canceled.cue, `${testCase.label} ${stage} Escape restores cue`).toBe(before.cue);
    expect(canceled.tutorial, `${testCase.label} ${stage} Escape restores tutorial`).toBe(before.tutorial);
    expect(canceled.active, `${testCase.label} ${stage} Escape returns source focus`)
      .toBe(`tile-${selectedCell.x}-${selectedCell.y}`);
    expect(canceled.roving, `${testCase.label} ${stage} Escape returns sole roving source`)
      .toEqual([`tile-${selectedCell.x}-${selectedCell.y}`]);
    expect(canceled.politeOwners, `${testCase.label} ${stage} Escape keeps one narrator`).toHaveLength(1);
    ...
    expect(canceled.board.height, `${testCase.label} ${stage} board height`)
      .toBe(testCase.viewport.width === 390 ? 378 : 600);
    expect(canceled.board.x, `${testCase.label} ${stage} mobile inset`)
  ...
  for (const testCase of cases) {
    test(testCase.label, async ({ browser }) => {
      ...
      try {
        await seedDeterministicMath(page, `keyboard-escape-${testCase.label}`);
        await openFreshNoReview(page, `keyboard-escape-${testCase.label}`);
        await expect(page.locator(".tile.idle-hint")).toHaveCount(2, { timeout: 2500 });
        ...
        await ordinaryTile.focus();
        await page.keyboard.press(testCase.activationKey);
        await expect(ordinaryTile).toHaveClass(/\bsel\b/);
        await page.keyboard.press("Escape");
        const ordinaryCanceled = await snapshot(page);
        expect(ordinaryCanceled.save, `${testCase.label} ordinary Escape has no save drift`)
          .toBe(ordinaryBefore.save);
        expect(ordinaryCanceled.moves, `${testCase.label} ordinary Escape spends no move`).toBe(4);
        expect(ordinaryCanceled.selected, `${testCase.label} ordinary Escape clears selection`).toEqual([]);
        expect(ordinaryCanceled.active, `${testCase.label} ordinary Escape returns focus`).toBe("tile-0-0");
        expect(ordinaryCanceled.roving, `${testCase.label} ordinary Escape returns sole roving focus`)
          .toEqual(["tile-0-0"]);
        expect(ordinaryCanceled.tiles, `${testCase.label} ordinary Escape retains 64 tiles`).toBe(64);
        expect(ordinaryCanceled.rows, `${testCase.label} ordinary Escape retains eight rows`).toBe(8);
        expect(ordinaryCanceled.disabled, `${testCase.label} ordinary Escape leaves board enabled`).toBe(0);
        expect(ordinaryCanceled.overflowX, `${testCase.label} ordinary Escape has no overflow`).toBe(0);
...
test("later off-order primary match stays truthful when its cascade advances the order", async ({ page }) => {
  ...
  expect(afterCascade.tutorial).not.toBe("Match the order flowers.");
  expect(afterCascade.tutorial).not.toBe("Match 3 fills the bouquet.");
...
test("guided Round 1 payoff keeps one dominant action", async ({ page }) => {
  ...
  await expect(page.locator("#tutorialPanel")).toBeVisible({ timeout: 7000 });
  ...
  expect(report.tutorialInViewport).toBe(false);
...
test("exact-mobile First Bouquet objectives stay readable without moving the altar", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  const consoleMessages = [];
  const pageErrors = [];
  const failedRequests = [];
  ...
  await seedDeterministicMath(page, "mobile-round-one-objective-fit");
  await openFreshNoReview(page, "mobile-round-one-objective-fit");

  const states = [
    { label: "initial", counts: [0, 0, 0, 0, 0, 0], moves: 6, expected: ["0/8", "0/6"], complete: [false, false] },
    { label: "ordinary progress", counts: [0, 3, 0, 0, 0, 4], moves: 4, expected: ["4/8", "3/6"], complete: [false, false] },
    { label: "Thorn Rose sealed", counts: [0, 5, 0, 0, 0, 8], moves: 2, expected: ["8/8", "5/6"], complete: [true, false] },
    { label: "Bone Star sealed", counts: [0, 6, 0, 0, 0, 7], moves: 2, expected: ["7/8", "6/6"], complete: [false, true] },
    { label: "both sealed", counts: [0, 6, 0, 0, 0, 8], moves: 1, expected: ["8/8", "6/6"], complete: [true, true] }
  ];
  ...
  for (const reducedMotion of ["no-preference", "reduce"]) {
    ...
    for (const stateCase of states) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        return {
          targets: targets.map((target) => {
            const name = target.querySelector(".objective-target-name");
            ...
            return {
              ...
              name: { text: name.textContent.trim(), bounds: bounds(name), fontSize: getComputedStyle(name).fontSize },
              count: { text: count.textContent.trim(), bounds: bounds(count), textBounds: textBounds(count), fontSize: getComputedStyle(count).fontSize },
              seal: seal ? { text: seal.textContent.trim(), bounds: bounds(seal) } : null
            };
          }),
      ...
      expect(geometry.targets.map((target) => target.count.text), `${label} visible counts`).toEqual(stateCase.expected);
      expect(geometry.targets.map((target) => target.complete), `${label} completion owners`).toEqual(stateCase.complete);
      ...
      geometry.targets.forEach((target, index) => {
        expect(target.scrollWidth, `${label} target ${index + 1} does not clip`).toBeLessThanOrEqual(target.clientWidth);
        expect(target.bounds.height, `${label} target ${index + 1} keeps baseline height`).toBeCloseTo(baselineTargetHeight, 1);
        ...
        for (const [part, partBounds] of [["name", target.name.bounds], ["count", target.count.bounds]]) {
          expect(partBounds.left, `${label} target ${index + 1} ${part} left inset`).toBeGreaterThanOrEqual(target.bounds.left + 1);
          expect(partBounds.right, `${label} target ${index + 1} ${part} right inset`).toBeLessThanOrEqual(target.bounds.right - 1);
          expect(partBounds.top, `${label} target ${index + 1} ${part} top inset`).toBeGreaterThanOrEqual(target.bounds.top + 1);
          expect(partBounds.bottom, `${label} target ${index + 1} ${part} bottom inset`).toBeLessThanOrEqual(target.bounds.bottom - 1);
        }
        if (stateCase.complete[index]) {
          expect(target.seal?.text, `${label} target ${index + 1} keeps completion meaning`).toBe("Sealed");
          expect(target.seal.bounds.left, `${label} target ${index + 1} seal left inset`).toBeGreaterThanOrEqual(target.bounds.left + 1);
          expect(target.seal.bounds.right, `${label} target ${index + 1} seal right inset`).toBeLessThanOrEqual(target.bounds.right - 1);
          expect(target.seal.bounds.top, `${label} target ${index + 1} seal top inset`).toBeGreaterThanOrEqual(target.bounds.top + 1);
          expect(target.seal.bounds.bottom, `${label} target ${index + 1} seal bottom inset`).toBeLessThanOrEqual(target.bounds.bottom - 1);
          expect(target.count.textBounds.right, `${label} target ${index + 1} count clears seal`).toBeLessThanOrEqual(target.seal.bounds.left - 1);
        } else {
          expect(target.seal, `${label} target ${index + 1} has no premature seal`).toBeNull();
        }
      });
      expect(geometry.targets[0].bounds.top, `${label} target tops align`).toBeCloseTo(geometry.targets[1].bounds.top, 1);
      expect(geometry.targets[0].bounds.bottom, `${label} target bottoms align`).toBeCloseTo(geometry.targets[1].bounds.bottom, 1);
      expect(geometry.targets[0].bounds.right, `${label} target chips stay separate`).toBeLessThanOrEqual(geometry.targets[1].bounds.left - 1);
      expect(geometry.targets[1].bounds.right, `${label} Moves stays separate`).toBeLessThanOrEqual(geometry.moves.bounds.left - 1);
      expect((geometry.moves.bounds.top + geometry.moves.bounds.bottom) / 2, `${label} Moves stays centered`).toBeCloseTo((geometry.targets[0].bounds.top + geometry.targets[0].bounds.bottom) / 2, 1);
      expect(geometry.board.top, `${label} objective progress does not move the altar`).toBeCloseTo(baselineBoardTop, 1);
      expect(geometry.board.width, `${label} altar width`).toBeCloseTo(378, 1);
      expect(geometry.board.height, `${label} altar height`).toBeCloseTo(378, 1);
