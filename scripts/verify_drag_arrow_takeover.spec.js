...
const PROFILES = [
  { label: "desktop-pointer-right-full", viewport: { width: 1280, height: 720 }, arrow: "ArrowRight", expected: "tile-2-0" },
  { label: "desktop-pointer-left-reduced", viewport: { width: 1280, height: 720 }, arrow: "ArrowLeft", expected: "tile-0-0", reduced: true },
  { label: "mobile390-touch-right-full", viewport: { width: 390, height: 844 }, arrow: "ArrowRight", expected: "tile-2-0", mobile: true },
  { label: "mobile390-touch-left-reduced", viewport: { width: 390, height: 844 }, arrow: "ArrowLeft", expected: "tile-0-0", mobile: true, reduced: true }
];
...
async function beginHeldPair(page, context, mobile) {
  ...
  if (!mobile) {
    ...
    return { start, end, client: null };
  }
  const client = await context.newCDPSession(page);
  await client.send("Input.dispatchTouchEvent", {
    type: "touchStart",
    touchPoints: [{ x: start.x, y: start.y, id: 17, radiusX: 2, radiusY: 2, force: 1 }]
  });
  await client.send("Input.dispatchTouchEvent", {
    type: "touchMove",
    touchPoints: [{ x: end.x, y: end.y, id: 17, radiusX: 2, radiusY: 2, force: 1 }]
  });
...
for (const profile of PROFILES) {
  test(`${profile.label}: arrow navigation takes over a held flower pair`, async ({ browser }) => {
    ...
    try {
      ...
      }, { key: SAVE_KEY, marker: `drag-arrow-takeover:${profile.label}` });
      await page.goto(`${BASE_URL}?drag-arrow-takeover=${profile.label}`, { waitUntil: "networkidle" });
      ...
      await page.keyboard.press(profile.arrow);
      const takeover = await report(page);
      expect(takeover.save, "keyboard takeover spends no move").toBe(before.save);
      ...
      expect(takeover.active).toBe(profile.expected);
      expect(takeover.roving).toEqual([profile.expected]);

      if (profile.label === "mobile390-touch-right-full") {
        await page.screenshot({ path: "work/drag-arrow-takeover-mobile390.png", fullPage: true });
      }
      ...
      expect(staleRelease.guideVisible).toBe(true);
      expect(staleRelease.active).toBe(profile.expected);
      expect(staleRelease.roving).toEqual([profile.expected]);

      const source = page.locator(`#${SOURCE_ID}`);
      const target = page.locator(`#${TARGET_ID}`);
      await source.focus();
