...
const CASES = [
  {
    label: "unfinished-round-one",
    state: { currentRound: 1, moves: 6, coins: 999 },
    expectedCoins: 0,
    expectedStage: "withered",
    expectedOrder: ["Exact reward", "+120 coins", "Then restore", "100 coins"],
    repaired: true
  },
  {
    label: "unfinished-round-two",
    state: {
      currentRound: 2,
      moves: 9,
      coins: 999,
    ...
    expectedCoins: 20,
    expectedStage: "restored",
    expectedOrder: ["Exact reward", "+150 coins", "Then upgrade", "120 coins"],
    repaired: true
  },
  {
    label: "unfinished-round-three",
    state: {
      currentRound: 3,
      moves: 7,
      coins: 999,
    ...
    expectedCoins: 50,
    expectedStage: "moonlit",
    expectedOrder: ["Exact reward", "+180 coins", "Then raise", "180 coins"],
    repaired: true
  },
  {
    label: "owned-replay-wallet",
    state: {
      currentRound: 1,
      moves: 6,
      coins: 777,
    ...
    expectedCoins: 777,
    expectedStage: "bloodroot",
    expectedOrder: ["Replay reward", "+120 coins", "Conservatory stays owned"],
    repaired: false
  }
];
...
for (const profile of PROFILES) {
  test(`active wallet requires earned receipts on ${profile.label}`, async ({ browser }) => {
    for (const testCase of CASES) {
      ...
      await page.addInitScript(({ key, value, marker }) => {
        ...
        key: SAVE_KEY,
        value: state,
        marker: `active-wallet-${profile.label}-${testCase.label}`
      });
      await page.goto(`${BASE_URL}?active-wallet=${profile.label}-${testCase.label}`, {
        waitUntil: "networkidle"
      });
      await expect(page.locator("#board .tile")).toHaveCount(64);

      const first = await report(page);
      expect(first.state.coins, testCase.label).toBe(testCase.expectedCoins);
      expect(first.stage, testCase.label).toBe(testCase.expectedStage);
      for (const copy of testCase.expectedOrder) {
        expect(first.order.toLowerCase(), `${testCase.label} shows ${copy}`).toContain(copy.toLowerCase());
      }
      if (testCase.repaired) {
        expect(first.ritual, testCase.label).toContain("Saved wallet repaired");
      } else {
        expect(first.ritual, testCase.label).not.toContain("Saved wallet repaired");
      }
