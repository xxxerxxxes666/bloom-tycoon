...
html_checks = {
    ...
    "function parseSavedPrimitiveNumber(value)": "saved numeric authority has one primitive gate",
    'typeof value === "string" && value.trim() !== ""': "numeric-string save migrations remain accepted",
    "numeric <= Number.MAX_SAFE_INTEGER": "numeric save fields reject unsafe finite magnitudes",
    "const savedRoundNumber = parseSavedPrimitiveNumber(state.currentRound);": "saved round identity rejects structured values",
