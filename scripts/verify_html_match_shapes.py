...
def verify_source_hooks():
    ...
    required = [
        ...
        "Math.max(1, Math.floor(Number(roundNumber) || 1))",
        "function maybeCompleteRound",
        "coins += streakPayout.total;",
        "const ownedReplayRecovery = isOwnedRestorationReplay();",
        "? coins + completionCoinPayout(plan).total",
        ...
        "swap-arrow-pulse",
        "function boardGuideTransientStateActive({ ignoreInvalidSwap = false } = {})",
        "function currentTutorialBoardGuidePair()",
        "function skippedRoundOneOpeningBoardGuidePair(plan = activeRoundPlan())",
        'stage: "skipped-opening"',
        "|| skippedRoundOneOpeningBoardGuidePair();",
        ...
        "function isFocusedHarvestHandoffActive(plan = activeRoundPlan())",
        "function primeFocusedHarvestHandoff(plan = activeRoundPlan())",
        '"focused-harvest-handoff-cue"',
        "body.focused-active-play.focused-harvest-handoff-cue .first-swap-cue",
        "round-one-refused",
        "Swap the glowing pair to open the next harvest.",
        "const continuingRefusal = invalidSwapTimer !== null",
        "let invalidSwapGeneration = 0",
        "const invalidSwapGenerationAtSchedule = ++invalidSwapGeneration",
        "if (invalidSwapGenerationAtSchedule !== invalidSwapGeneration)",
        "invalidSwapReturnCue",
        "invalidSwapReturnHintCells",
        "function restoreUntouchedRoundOneOpeningGuideAfterRefusal()",
        "&& restoreUntouchedRoundOneOpeningGuideAfterRefusal();",
        "restoredThornLesson || restoredUntouchedRoundOneGuide",
        ...
        'panel.classList.toggle("failed-tutorial", failedTutorial)',
        'const failedTutorial = tutorialActive && isBouquetFailed();',
        '? "RETRY"',
        "!tutorialActive || focusedPayoffOwnsCommand()",
        ...
        'aria-live="${moveUrgencyPulseActive && lowMoves && !shuffleOutcomeNarrationPending ? "polite" : "off"}"',
        ...
        "function isRoundTwoThornHandoffActive(plan = activeRoundPlan())",
        "restoredRoundTwoGuideMoves === 1",
        "isRoundTwoThornHandoffActive() && idleHintCells.size !== 2",
        "const ROUND_ONE_AGENCY_HINT_DELAY_MS = 7000;",
        "const TARGET_LITERACY_DURATION_MS = 1500;",
        "function beginRoundOneAgencyDiscovery",
        ...
        "const roundOneAgencyHint = isRoundOneAgencyHandoffActive();",
        "const roundOneOrderCorrection = isRoundOneOrderCorrectionActive();",
        "function isRoundOneOrderCorrectionActive(plan = activeRoundPlan())",
        "let roundOneLastSwapMissedOrder = false",
        "function roundOneSwapGainedOrderFlowers(result, plan = activeRoundPlan())",
        "function roundOneHasExploratoryHarvest(plan = activeRoundPlan())",
        "const restoredRoundOneExplorationGuide = currentRound === 1",
        "roundOneLastSwapMissedOrder = currentRound === 1",
        "const roundOneAgencyHintAfterDelay = isRoundOneAgencyHandoffActive();",
        "const roundOneOrderCorrectionAfterDelay = isRoundOneOrderCorrectionActive();",
        "const roundOneAgencyQuietWindowReady = roundOneAgencyHintAfterDelay",
        "const recoverRoundOneAgencySelection = roundOneAgencyQuietWindowReady",
        "&& !recoverRoundOneAgencySelection",
        "if (!roundOneAgencyHintAfterDelay)",
        "function alignPassiveRoundOneRovingEntry(source, plan = activeRoundPlan())",
        "active === document.body",
        "active === document.documentElement",
        "alignPassiveRoundOneRovingEntry(move[0], plan);",
        "alignPassiveRoundOneRovingEntry(a, plan);",
        "Match Thorn Rose.",
        "Match the order flowers.",
        ...
        'const mobileRoundContext = roundComplete',
        ': plan.name;',
        '<span class="objective-mobile-verb"> · ${mobileRoundContext}</span>',
        '<span class="objective-mobile-verb"> · First Bouquet</span>',
        "data-slot-progress",
        ...
        "function bouquetCompositionLayout(index, total)",
        'data-unit-index="${unitIndex}"',
        'data-crafted-bloom-count="${entries.length}"',
        "id=\"liveBouquetAssembly\"",
        "bouquet-assembly-ring",
        "crafted-bloom-bind",
        "const BOUQUET_BINDING_HOLD_MS = 1720;",
        "const OWNED_REPLAY_BINDING_HOLD_MS = 450;",
        "const OWNED_REPLAY_VISIBLE_BINDING_MS = 250;",
        "const OWNED_REPLAY_FINAL_TRANSFER_MS = 520;",
        "const OWNED_REPLAY_FINAL_RENEWAL_MS = 400;",
        "? OWNED_REPLAY_BINDING_HOLD_MS",
        ": BOUQUET_BINDING_HOLD_MS;",
        "trophy.dataset.assemblyState = bouquetSealPulseActive ? \"forming\" : \"assembled\";",
        "crafted-vine",
        "dataset.assemblyReady",
        "objective-target-seal",
        'const accessibleLabel = `${flower[0]}, ${displayCount} of ${needed}${completeClass ? ", sealed" : ""}`;',
        'role="group" aria-label="${accessibleLabel}"',
        'class="objective-target-name" data-compact-label="${compactFlowerLabel(flowerId)}" aria-hidden="true"',
        'const completeClass = thornGoalComplete(plan) ? "complete" : "";',
        'Cursed Thorn, ${progress} of ${needed}${completeClass ? ", sealed" : ""}',
        'data-thorn-objective="true" role="group" aria-label="${stateLabel}"',
        ...
        "const FOCUSED_ECONOMY_SAVE_VERSION = 3;",
        "function normalizeSavedMoveBudget(value, plan)",
        "function savedMoveBudgetNeedsRepair(value, normalized)",
        "moves = normalizeSavedMoveBudget(state.moves, focusedPlan);",
        'const SAVED_MOVE_REPAIR_RECEIPT_COPY = "MOVES RESTORED · BOUQUET PROGRESS KEPT.";',
        'const SAVED_PROGRESS_REPAIR_RECEIPT_COPY = "SAVED PROGRESS REPAIRED · INVALID VALUES CLEARED.";',
        "function focusedEconomyBalanceForProgress()",
        "function recoverCompletedFocusedOrder()",
        ...
        "function parseSavedPrimitiveNumber(value)",
        "const numeric = parseSavedPrimitiveNumber(value);",
        ...
        "function savedBooleanNeedsRepair(value, normalized)",
        'const savedRoundComplete = savedBoolean("roundComplete");',
        ...
        "const savedRoundNumber = parseSavedPrimitiveNumber(state.currentRound);",
        "const savedCoins = parseSavedPrimitiveNumber(state.coins);",
        "const savedRestoredRoundTwoGuideMoves = parseSavedPrimitiveNumber(state.restoredRoundTwoGuideMoves);",
        "const x = parseSavedPrimitiveNumber(thorn?.x);",
        "const y = parseSavedPrimitiveNumber(thorn?.y);",
        "if (recoverCompletedFocusedOrder())",
        "if (!hasAuthoritativeOwnedWallet)",
        "savedFocusedEconomyVersion < 3",
        "coins += completionCoinPayout(focusedPlan).total;",
        ...
        "top: 31px;",
        "body.restored-greenhouse-handoff.round-three-focus-active .title {",
        ...
        "coins += streakPayout.total;",
        "const hasAuthoritativeOwnedWallet = conservatoryFullyOwned()",
        "Replay reward · +${plan.reward.coins} coins banked · ${coins} coins in wallet.",
        "function isPostOnboardingIdleHintActive(plan = activeRoundPlan())",
        "plan.round >= 2",
        ...
        "if (!completedBySkip && !selectedReplayHintPair.length && (skippingRoundOneAgency || restartPostOnboardingHint))",
        "const untouchedRoundThreeOpening = plan.round === 3",
        "&& (hasMadeValidMove || untouchedRoundThreeOpening)",
        ...
        "Math.max(80, (finalHarvestRenewal ? 480 : 520) - finalHarvestLead)",
        "finalHarvestRenewal ? OWNED_REPLAY_FINAL_RENEWAL_MS : 520",
        "finalHarvestRenewal ? OWNED_REPLAY_FINAL_TRANSFER_MS : 660",
        "const OWNED_REPLAY_BINDING_HOLD_MS = 450;",
        'panel.dataset.ownedRenewalPhase = ceremony.ownedReplay ? ownedRenewalPhase || "settled" : "";',
        '"REPLAY REWARD BANKED · CONSERVATORY REMAINS 100% RAISED"',
        ...
        ".owned-replay-renewal[data-renewal-phase=\"transfer\"] .owned-renewal-reserve",
        "data-reserve-index=\"${index}\"",
        "+${payout.total} replay coins banked. Wallet ${coins}.",
        ...
        "isRestoredRoundTwoWindow() || failedFocusedRestoredOrder || (finalHarvestLanding && currentRound === 2)",
        ...
        "clip-path: inset(50%);",
        "order-contract-outcome",
        "data-contract-restoration-cost",
        "contract-ingredient-deficit",
        ...
        "No bloom — no match.",
        "body.focused-active-play:not(.tutorial-active):not(.round-one-complete) .first-swap-cue.swap-refused",
        ".first-swap-cue.swap-refused::before { content:\"NO BLOOM\"; }",
        "body.focused-active-play:not(.tutorial-active) .tutorial-command-region > .first-swap-cue.swap-refused",
        'return isRoundOneAgencyHandoffActive()',
        '"Try another adjacent swap."',
        'panel.classList.toggle("failed-tutorial", failedTutorial)',
        ...
        "const tutorialPanelOwnsNarration = focusedSliceRound(plan)",
        "tutorialPanelOwnsNarration || isBouquetFailed(plan) || finalHarvestNarration",
        ...
        "thornOutcomeStillOwnsFocus",
        "settledOutcomeMayRestoreBoardFocus",
        'if (/^tile-\\d+-\\d+$/.test(postRenderFocusTarget))',
        "resolvedBoardMayRestoreFocus",
        ...
        "function invalidSwapFeedbackActive()",
        "const continuingRefusal = invalidSwapTimer !== null;",
        "const repeatedSingleCellRefusal = a.x === b.x && a.y === b.y;",
        "const pairedRefusal = priorCell",
        "const refusalSource = pairedRefusal ? priorCell : a;",
        "|| Boolean(document.querySelector(\"#board .tile.invalid-swap\"))",
        "const retireRefusal = invalidSwapFeedbackActive();",
        "ordinaryRefusalHelpReplayActive = [2, 3].includes(Number(currentRound)) && retireRefusal;",
        ...
        "function untouchedRoundOneOpeningGuidePair",
        "function boardGuideTransientStateActive({ ignoreInvalidSwap = false } = {})",
        "const authoredOpeningRefusalActive = invalidSwapTimer !== null",
        "boardGuideTransientStateActive({ ignoreInvalidSwap: authoredOpeningRefusalActive })",
        "const untouchedOpeningPair = untouchedRoundOneOpeningGuidePair();",
        "&& untouchedOpeningPair.some((pairCell) => pairCell.x === selected.x && pairCell.y === selected.y)",
        "selectedOpeningCell || { x, y }",
        "function cancelBoardSelection",
        "selected && selected.x === x && selected.y === y",
        'if (event.key === "Escape" && selected)',
        "tap(boardFocusCell.x, boardFocusCell.y, { keyboardSelection: true })",
        "if (options.keyboardSelection && tutorialStartTimer && window.clearTimeout)",
        "const selectedKeyboardPair = selectedGuidedHintPair();",
        "const selectedArrowConfirmsFocusedCounterpart = selected",
        "selected.x + delta.x === cell.x",
        "selected.y + delta.y === cell.y",
        "const selectedKeyboardCounterpart = selectedKeyboardPair.find",
        "boardFocusCell = clampBoardFocusCell(selectedKeyboardCounterpart);",
        ...
        "function isObjectiveUsefulIdleHintPair",
        "function shouldPreserveIdleHintDuringKeyboardNavigation",
        "const ordinaryReplayAuthority = tutorialActive && tutorialReplayOwnsNarration;",
        "|| isObjectiveUsefulIdleHintPair(hintCells, cell)",
        "if (!keyboardTookOverDrag && !shouldPreserveIdleHintDuringKeyboardNavigation())",
        "const selectedReplayFocus = selected && selectedReplayHintPair.find",
        "|| selectedReplayFocus",
        "[2, 3].includes(plan.round) && selectedGuidedHintPair().length === 2",
        'return "Choose the other glowing flower.";',
        "&& !tutorialReplayOwnsNarration",
        "const counterpart = hintedPairCells().find",
        "return `Tap ${flowers[board[counterpart.y][counterpart.x]][0]} ${direction}.`;",
        'document.body.classList.toggle("selected-guided-help", Boolean(selectedHelpCounterpartKey))',
        'document.body.classList.toggle("selected-guided-play", Boolean(selectedPlayCounterpartKey))',
        'const selectedGuidedCounterpartKey = selectedHelpCounterpartKey || selectedPlayCounterpartKey',
        'const selectedGuidedAuthority = Boolean(selectedGuidedCounterpartKey)',
        'const guidedCounterpartClass = selectedGuidedCounterpartKey === fallKey',
        'legalClass && (!selectedGuidedAuthority || guidedCounterpartClass)',
        'selectedClass\n              ? " guided exchange source"',
        'guidedCounterpartClass ? " guided exchange destination"',
        '!selectedGuidedAuthority && targetMatchEndpointIndex >= 0',
        "body.selected-guided-help .tile.legal-target:not(.guided-counterpart)",
        "body.selected-guided-help .tile.match-preview:not(.guided-counterpart)",
        "body.selected-guided-help .tile.guided-counterpart",
        ...
        'id="board" role="grid"',
        '<button class="renew-btn" id="renewBtn" type="button">Next Bouquet</button>',
        'id="mobileGreenhousePlinth"',
        'id="renewBtn" type="button">Next Bouquet</button>\n      <div class="mobile-greenhouse-plinth"',
        ...
        "const TUTORIAL_COMMAND_SETTLED_GUARD_FRAMES = 8",
        "function armPayoffHandoffInputGuard(",
        "duration = PAYOFF_HANDOFF_INPUT_GUARD_MS",
        "keyboardOnly = false",
        "payoffHandoffInputGuardKeyboardOnly = keyboardOnly",
        "payoffHandoffInputGuardGeneration += 1",
        "settledFrames >= TUTORIAL_COMMAND_SETTLED_GUARD_FRAMES",
        "function retirePayoffHandoffInputGuard()",
        "payoffHandoffInputGuardGeneration += 1",
        "guardedGeneration !== payoffHandoffInputGuardGeneration",
        "retirePayoffHandoffInputGuard();",
        "const keyboardOwnedClick = event?.type === \"click\" && event.detail === 0",
        "guardedGeneration !== payoffHandoffInputGuardGeneration",
        "&& (!activeFocusId || activeFocusId === focusTargetId)",
        ...
        "TUTORIAL_COMMAND_INPUT_GUARD_MS",
        "if (retryingFailedBouquet)",
        'armPayoffHandoffInputGuard("", TUTORIAL_COMMAND_INPUT_GUARD_MS, true)',
        "else if (restoredGreenhouseNextOrder || secondUpgradeNextOrder)",
        ...
        "let bloomAudioWindowFocused = document.hasFocus()",
        "let bloomAudioSuspendedForInterruption = false",
        "let bloomAudioVisibilityTransition = Promise.resolve()",
        "let bloomAudioGestureResume = null",
        ...
        "function bloomAudioInterrupted()",
        "function resumeBloomAudioFromOwnedInterruption(context)",
        "function syncBloomAudioVisibility()",
        "contextCreated",
        "await context.resume()",
        "await context.suspend()",
        "syncBloomAudioVisibility()",
        ...
        "const hintedPair = settledOutcomeOwnsBoardGuidance ? [] : hintedPairCells();",
        "const semanticLabels = [",
        "row ${y + 1}, column ${x + 1}",
        'iconMarkup(value, "", "", "tile-icon-wrap")',
        "--tile-color",
        ...
        "if (currentRound === 1 && !tutorialSkipped && !blackCandleLessonComplete)",
        ...
        "function pulseBouquetSealFeedback(completionLead = 0)",
        "finalHarvestCeremonyPacing = boundedLead > 0",
        ...
        "roundThreeConservatoryRaised = savedRoundThreeConservatoryRaised || currentRound > 3",
        "freshConservatorySettlement,",
        "freshConservatorySettlement = savedFreshConservatorySettlement",
        "&& currentRound === 3",
        "&& roundComplete",
        "&& conservatoryFullyOwned();",
        "const roundThreeFinalPending = currentRound === 3 && roundComplete && !finalHarvestLanding && !roundThreeConservatoryRaised;",
        "const roundThreeFinalPayoff = roundThreeFinalPending || roundThreeFinalComplete;",
        "function finalHarvestEligibility",
        "function directObjectiveMoveDetail",
        "function startFinalHarvestCompletionTransfer",
        "function revealFinalHarvestCeremony",
        "data-final-harvest-slot",
        ...
        "--dial-pct",
        "function pulseActiveGreenhouseIntake(options = {})",
        ...
        "body.owned-replay-match-intake .greenhouse-restoration-dial",
        "queueGreenhouseIntakeFlights(",
        "pulseActiveGreenhouseIntake({ ownedReplayMatch: true });",
        ...
        'id="coinBalance"',
        'id="coins"',
        "function updateFocusedPayoffLiveRegionOwnership",
        "function focusedPayoffOwnsCommand",
        "const settledAcceptedHarvestPending = acceptedHarvestCommitActive",
        "&& !isResolving\n        && Boolean(pendingSettledBoardOutcome)",
        "Boolean(settledBoardOutcome) || settledAcceptedHarvestPending",
        "settledBoardOutcomeOwnsNarration",
        "settledBoardOutcomeRevealFrame",
        "function settledBoardOutcomeForResult",
        "function acceptedHarvestCommitText",
        ...
        "const settledOutcomeOwnsBoardGuidance = settledBoardOutcomeOwnsBoardGuidance(activePlan);",
        "const settledOutcomeOwnsGuidance = settledBoardOutcomeOwnsBoardGuidance();",
        "if (settledBoardOutcomeOwnsBoardGuidance())",
        ...
        "updateFocusedPayoffLiveRegionOwnership(activePlan)",
        ...
        "roundTwoGreenhouseUpgraded = savedRoundTwoGreenhouseUpgraded || currentRound > 2",
        ...
        "const nextOrderHandoffOwnsNarration",
        'nextOrderHandoffOwnsNarration ? "polite" : "off"',
        "body.restored-greenhouse-handoff .tutorial-panel { display:none!important; }",
        "function finishRestoredGreenhouseHandoff()",
        "if (restoredGreenhouseNextOrder) {",
        "startRestoredGreenhouseHandoff();",
        ...
        "writeTargetLegalMovePatch(0, 0, targetIds[0], plan.round === 3 ? 4 : 3)",
        "writeTargetLegalMovePatch(4, 0, targetIds[1], plan.round === 3 ? 3 : 4)",
    ...
    required.extend([
        ...
        "id=\"bouquetRewardPromise\"",
        'data-slot-state="${slotState}"',
        'data-next-objective="${nextObjectiveSlot === index}"',
        "assembly.dataset.visibleBlooms",
        "function updateBouquetProgress",
        "Complete for ${plan.reward.coins} coins",
        "Bank ${plan.reward.coins} · Wallet ${ownedReplayWalletAfterBouquet}",
        "${plan.reward.coins} replay coins bank after the bouquet; wallet becomes ${ownedReplayWalletAfterBouquet}; Conservatory stays owned",
        'rewardLabel: alreadyOwned ? "Replay reward" : "Exact reward"',
        'consequenceCopy: alreadyOwned ? `${replayWallet} coins` : milestone.action',
        ...
        "Bouquet · ${progress.earned}/${progress.needed}",
        "Bouquet Complete · ${progress.earned}/${progress.needed}",
        'data-progress-authority="bouquet-order"',
        "text-overflow: clip",
        "white-space: nowrap",
        "id=\"tutorialPanel\"",
        "id=\"tutorialHelpBtn\"",
        ...
        "const selectedReplayHintPair = replay",
        "function selectedGuidedHintPair(hintCells = idleHintCells, cell = selected)",
        "|| isRoundOneAgencyHandoffActive()",
        "const selectedIsGuided = Boolean(selectedKey)",
        "if (selected && !selectedIsGuided)",
        "if (replayGuideSource)",
        "function skipTutorial",
        "const authoredLessonPending = plan.round === 1",
        "&& !tutorialSkipped",
        "&& !blackCandleLessonComplete",
        "(plan.round === 1 && !tutorialSkipped && !blackCandleLessonComplete && !armedLineRelic)",
        "function scheduleFreshTutorial",
        "Swap the glowing flowers.",
        "Choose the other glowing flower.",
        "selected-continuation",
        "invalidSwapReturnSelection",
        "Match 3 fills the bouquet.",
        "Match 4 Bone Stars to arm Black Candle Vine.",
        "authoredBlackCandleReplayPair",
        "restoredAuthoredBlackCandleReplayPair",
        ...
        "function savedArmedLineRelicNeedsRepair(candidate, normalized)",
        "savedFlowerId >= 0",
        "savedFlowerId < flowers.length",
        "|| savedArmedLineRelicNeedsRepair(state.armedLineRelic, normalizedArmedLineRelic)",
        ...
        "const selectedOpeningCell = selected",
        ...
        "const boundaryArrowNoop = target.x === cell.x && target.y === cell.y",
        "if (boundaryArrowNoop)",
        "function freshTutorialEligible()",
        'if (event.key === "Escape" && boardDragInputActive())',
        "cancelInterruptedBoardDrag();",
        "restoreInterruptedBoardDragFocus();",
        "const keyboardTookOverDrag = boardDragInputActive()",
        "if (keyboardTookOverDrag) {\n        cancelInterruptedBoardDrag();\n        restoreInterruptedBoardDragFocus();\n      }\n      const selectedArrowConfirmsFocusedCounterpart = selected",
