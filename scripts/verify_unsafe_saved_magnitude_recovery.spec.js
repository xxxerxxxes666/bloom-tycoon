...
const MALFORMED_PROGRESS_PROFILES = [
  { label: "unsafe-magnitude", value: 1e100 },
  { label: "structured-array", value: [999] }
];
...
for (const profile of MALFORMED_PROGRESS_PROFILES) {
  for (const testCase of CASES) {
    test(`${profile.label} replay progress returns to an honest active order on ${testCase.label}`, async ({ browser }) => {
      ...
      try {
        await page.addInitScript(({ key, state, marker }) => {
          ...
          state: {
            ...
            moves: 6,
            coins: profile.value,
            counts: [0, profile.value, 0, 0, 0, profile.value],
            cursedThorns: [],
          ...
          marker: `malformed-saved-progress-${profile.label}-${testCase.label}`
        });
        await page.goto(`${BASE_URL}?malformed-saved-progress=${profile.label}-${testCase.label}`, { waitUntil: "networkidle" });
        ...
        await page.screenshot({
          path: `work/malformed-saved-progress-${profile.label}-repaired-${testCase.label}.png`,
          fullPage: false
        });
